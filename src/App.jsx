...
// --- 🔥 IMPORT FINANCE (ARAHKAN KE FOLDER BARU) ---
import AdminFinance from './pages/admin/finance/index'; 

// --- 🔥 IMPORT SISWA (BYPASS LANGSUNG KE FILE DALAM FOLDER) ---
...
function AdminStudentsWrapper({ db }) {
  ...
  return (
    <div className="w-full">
      {view === 'list' && (
        <StudentList db={db} students={students} classLogs={classLogs} onSelect={handleDetail} onCreate={handleCreate} />
      )}
      ...
      {view === 'form' && (
        <StudentForm db={db} initialData={selectedStudent} onCancel={handleBack} onSuccess={handleBack} />
      )}
    </div>
  );
}
  ...
  useEffect(() => {
    ...
    const unsubStudents = onSnapshot(collection(db, "students"), s => {
      ...
      setExpiringPackages(all.filter(std => { 
        ...
        const diff = Math.ceil(Math.abs(new Date() - parseDate(std.createdAt)) / (1000 * 60 * 60 * 24)); 
        return diff >= 25; 
      }));
    });
