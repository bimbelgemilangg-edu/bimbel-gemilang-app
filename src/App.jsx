...
import { Bell, Clock, DollarSign, Users, GraduationCap, Calendar, AlertTriangle, Settings, Menu, LogOut, TrendingUp, BarChart3, ExternalLink } from 'lucide-react';

// --- IMPORT HALAMAN UTAMA SAJA (Tanpa Finance) ---
...
const DashboardAdmin = ({ onLogout }) => {
  ...
  // Ganti link di bawah ini dengan Link Spreadsheet Anda
  const GOOGLE_SHEET_URL = "https://docs.google.com/spreadsheets/u/0/"; 

  const handleMenuClick = (id) => {
    if (id === 'finance') {
      // Membuka Tab Baru ke Google Sheet
      window.open(GOOGLE_SHEET_URL, '_blank');
    } else {
      // Pindah Halaman Aplikasi
      setView(id);
    }
      ...
      <div className={`fixed inset-y-0 left-0 transform ${isSidebarOpen ? "translate-x-0" : "-translate-x-full"} md:relative md:translate-x-0 transition-transform z-40 w-72 bg-white border-r h-full flex flex-col shadow-xl md:shadow-none`}>
        ...
        <nav className="p-6 space-y-2 flex-1 overflow-y-auto">
          {[
            ...
            {id:'finance',l:'Laporan Keuangan',i:ExternalLink}, // Icon Panah Keluar
            {id:'settings',l:'Pengaturan',i:Settings}
          ].map(m => (
            <button key={m.id} onClick={()=>handleMenuClick(m.id)} className={`w-full flex items-center gap-4 px-5 py-4 rounded-2xl text-sm font-black transition-all ${view===m.id?'bg-blue-600 text-white shadow-xl translate-x-2':'text-gray-400 hover:bg-gray-50'}`}>
              <m.i size={20}/> {m.l}
            </button>
          ))}
        </nav>
