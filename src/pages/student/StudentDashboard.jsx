...
import { 
  ...
  Megaphone, TrendingUp, Trophy, ArrowRight, Maximize2
} from 'lucide-react';
...
const StudentDashboard = () => {
  ...
  useEffect(() => {
    ...
    const fetchAll = async () => {
      ...
      try {
        ...
        // ➕ POSTERS: Filter hanya untuk Siswa & Semua
        let fetchedPosters = [];
        try {
  ...
  const renderDashboardHome = () => (
    <div style={st.contentWrapper}>
      ...
      {/* ➕ SLIDE BERITA - FILTERED & CLICKABLE */}
      {posters.length > 0 && (
              ...
              <SwiperSlide key={post.id} onClick={() => setSelectedNews(post)}>
                <div style={{ ...st.slideCard, backgroundImage: `url(${post.imageUrl})`, cursor: 'pointer' }}>
                  <div style={st.slideOverlay}>
                    ...
                    {!isMobile && <p style={st.slideDesc}>{post.content?.substring(0, 120) || "Klik untuk baca selengkapnya"}</p>}
                    <button style={{ marginTop: isMobile ? 6 : 12, background: 'rgba(255,255,255,0.2)', color: 'white', border: '1px solid rgba(255,255,255,0.3)', padding: '6px 14px', borderRadius: 8, fontSize: 10, fontWeight: 700, cursor: 'pointer', display: 'flex', alignItems: 'center', gap: 4, backdropFilter: 'blur(4px)' }}>
                      <Maximize2 size={12} /> Baca Selengkapnya
                    </button>
                  </div>
                </div>
              </SwiperSlide>
      ...
      {/* RINGKASAN RAPORT */}
      {raportSummary && (
        <div onClick={() => navigate('/siswa/smart-rapor')} style={{ background: 'linear-gradient(135deg, #667eea 0%, #764ba2 100%)', borderRadius: 16, padding: 20, color: 'white', cursor: 'pointer', transition: 'transform 0.2s', border: '1px solid rgba(255,255,255,0.2)' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
              <Trophy size={28} color="#fbbf24" />
              <div><h3 style={{ margin: 0, fontSize: 16, fontWeight: 800 }}>📊 Ringkasan Raport</h3><p style={{ margin: '4px 0 0', fontSize: 11, opacity: 0.85 }}>Periode {raportSummary.periode?.replace('-', ' / ')}</p></div>
            </div>
            <ArrowRight size={20} />
          </div>
          <div style={{ display: 'flex', gap: 20, marginTop: 16, flexWrap: 'wrap' }}>
            {raportSummary.nilaiAkhir !== null ? (
              <div style={{ textAlign: 'center' }}><div style={{ fontSize: 32, fontWeight: 900 }}>{raportSummary.nilaiAkhir}</div><div style={{ fontSize: 10, opacity: 0.8 }}>Nilai Akhir</div></div>
            ) : (
              <div style={{ textAlign: 'center' }}><div style={{ fontSize: 13, fontWeight: 600 }}>Belum tersedia</div><div style={{ fontSize: 10, opacity: 0.7 }}>Generate dulu ya</div></div>
            )}
          </div>
        ...
        <div style={{ background: 'white', padding: 20, borderRadius: 15, border: '1px solid #e2e8f0', borderTop: '4px solid #3b82f6' }}>
          <h3 style={{ margin: '0 0 15px', fontSize: 16, fontWeight: 800, display: 'flex', alignItems: 'center', gap: 8 }}>
            ...
            {activeSurveys.filter(s => !filledSurveys[s.id]).length > 0 && (
              <span style={{ background: '#ef4444', color: 'white', padding: '2px 8px', borderRadius: 20, fontSize: 10 }}>{activeSurveys.filter(s => !filledSurveys[s.id]).length} pending</span>
            )}
          </h3>
      ...
      <div style={isMobile ? st.mainGridMobile : st.mainGrid}>
        <div style={st.leftColumn}>
          ...
          <section style={st.sectionCard}>
            ...
            {tasks.length === 0 ? <div style={st.emptyState}>📭 Belum ada tugas.</div> : tasks.map((task, i) => {
              const dl = getAssignmentDeadline(task);
              return (
                <div key={i} style={{ ...st.taskItem, borderLeftColor: isDeadlinePassed(dl) ? '#ef4444' : isDeadlineSoon(dl) ? '#f59e0b' : '#9b59b6' }}>
                  <div style={{ flex: 1 }}><div style={{ fontSize: 14, fontWeight: 700 }}>{task.title}</div>{dl && <div style={{ fontSize: 11, marginTop: 4 }}><Clock size={12} /> {formatDateOnly(dl)}</div>}</div>
  ...
  return (
    <div style={st.mainContainer}>
      ...
      {/* ➕ MODAL BERITA - FULL CONTENT */}
      {selectedNews && (
        <div style={st.modalOverlay} onClick={() => setSelectedNews(null)}>
          <div style={{ ...st.modalContent, width: isMobile ? '92%' : '600px', maxHeight: isMobile ? '85vh' : '80vh', overflowY: 'auto' }} onClick={e => e.stopPropagation()}>
            <button onClick={() => setSelectedNews(null)} style={{ position: 'sticky', top: 10, right: 10, float: 'right', zIndex: 1, background: 'rgba(0,0,0,0.6)', color: 'white', border: 'none', borderRadius: '50%', width: 32, height: 32, cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center' }}><X size={18} /></button>
            <img src={selectedNews.imageUrl} style={{ width: '100%', maxHeight: isMobile ? 200 : 300, objectFit: 'cover', borderRadius: '12px 12px 0 0' }} alt="" />
            <div style={{ padding: isMobile ? 16 : 24 }}>
              <span style={{ fontSize: 10, color: '#e11d48', fontWeight: 800, background: '#fff1f2', padding: '4px 10px', borderRadius: 20 }}>📢 Info Resmi</span>
              <h2 style={{ margin: '12px 0 8px', fontSize: isMobile ? 18 : 22, fontWeight: 800 }}>{selectedNews.title}</h2>
              <p style={{ fontSize: isMobile ? 13 : 15, color: '#475569', lineHeight: 1.7, whiteSpace: 'pre-wrap' }}>{selectedNews.content || selectedNews.desc || "Tidak ada deskripsi tambahan."}</p>
              {selectedNews.createdAt && (
                <div style={{ marginTop: 16, fontSize: 11, color: '#94a3b8', borderTop: '1px solid #e2e8f0', paddingTop: 12 }}>
                  📅 Dipublikasikan: {selectedNews.createdAt.toDate ? new Date(selectedNews.createdAt.toDate()).toLocaleDateString('id-ID', { day: 'numeric', month: 'long', year: 'numeric' }) : '-'}
                </div>
              )}
            </div>
          </div>
