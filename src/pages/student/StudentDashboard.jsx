// src/pages/student/StudentDashboard.jsx - VERSI PERBAIKAN
...
const StudentDashboard = () => {
  ...
  // QR SCANNER (sama seperti kode Anda)
  useEffect(() => {
    let qr = null;
    const start = async () => {
      try {
  ...
  useEffect(() => {
    ...
    const fetchAll = async () => {
      ...
      try {
        ...
        } catch (e) { console.warn('Fetch posters error:', e); }
        try {
        ...
        try {
          const moduls = (await getDocs(query(collection(db, "bimbel_modul"), where("targetKategori", "in", ["Semua", curKat]), orderBy("createdAt", "desc"), limit(10))))
            .docs.map(d => ({ id: d.id, ...d.data() }));
          setTasks(moduls.filter(m => m.quizData?.length > 0 || m.blocks?.some(b => b.type === 'assignment')).slice(0, 5));
        } catch (taskErr) { console.warn('Fetch tasks error:', taskErr.message); setTasks([]); }
      ...
      } catch (err) { console.error('Error fetch dashboard:', err); } finally { setLoading(false); }
    };
    fetchAll();
  }, [authReady, studentId]);
  ...
  const submitSurvey = async () => {
    ...
    } catch (err) { console.error('Error submit survey:', err); alert("❌ Gagal mengirim survei: " + err.message); } finally { setSurveyLoading(false); }
  };
  ...
  const renderDashboardHome = () => (
    <div style={st.contentWrapper}>
      ...
      {/* CAROUSEL POSTER - TANPA SUSPENSE */}
      {posters.length > 0 && (
      ...
      {/* SECTION SURVEI - SAMA SEPERTI KODE ANDA */}
      ...
      {/* MAIN GRID - SAMA SEPERTI KODE ANDA */}
        ...
        <div style={st.leftColumn}>
          ...
          <section style={st.sectionCard}>
            ...
            {tasks.length === 0 ? <div style={st.emptyState}>📭 Belum ada tugas.<br /><span style={{ fontSize: 10, color: '#94a3b8' }}>Tugas akan muncul setelah guru membuatnya.</span></div> : tasks.map((task, i) => {
              ...
              return (<div key={i} style={{ ...st.taskItem, borderLeftColor: passed ? '#ef4444' : soon ? '#f59e0b' : '#9b59b6' }}><div style={{ flex: 1 }}><div style={{ fontSize: 14, fontWeight: 700 }}>{task.title}</div>{dl && <div style={{ fontSize: 11, marginTop: 4 }}><Clock size={12} /> {passed ? '⛔ Terlewat ' : soon ? '⚠️ Segera ' : '📅 '}{formatDateOnly(dl)}</div>}</div><button onClick={() => setActiveMenu('materi')} style={st.btnTaskBuka}>Buka</button></div>);
            })}
          </section>
        </div>
  ...
  return (
    <div style={st.mainContainer}>
      ...
      {isScanning && (<div style={st.modalOverlay} onClick={() => setIsScanning(false)}><div style={st.qrModalContent} onClick={e => e.stopPropagation()}><button onClick={() => setIsScanning(false)} style={st.btnCloseModal}><X size={20} /></button><h3>Scan QR Code Absensi</h3><div id="reader" style={{ width: '100%', borderRadius: 15, overflow: 'hidden' }}></div><p style={{ fontSize: 11, color: '#64748b', marginTop: 12 }}>Arahkan kamera ke QR Code yang ditampilkan guru</p></div></div>)}
      {showSurveyModal && currentSurvey && (<div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.8)', zIndex: 3000, display: 'flex', justifyContent: 'center', alignItems: 'center', padding: 20 }}><div style={{ background: 'white', borderRadius: 20, padding: 30, width: '100%', maxWidth: 550, maxHeight: '85vh', overflowY: 'auto', position: 'relative' }}><button onClick={() => setShowSurveyModal(false)} style={{ position: 'sticky', top: 0, right: 0, float: 'right', background: '#f1f5f9', border: 'none', borderRadius: '50%', width: 32, height: 32, cursor: 'pointer' }}><X size={18} /></button><div style={{ textAlign: 'center', marginBottom: 20, clear: 'both' }}><span style={{ fontSize: 40 }}>📋</span><h2 style={{ margin: '10px 0 5px', fontSize: 20 }}>{currentSurvey.title}</h2>{currentSurvey.isRequired && <span style={{ display: 'inline-block', background: '#fee2e2', color: '#ef4444', padding: '4px 12px', borderRadius: 20, fontSize: 11, fontWeight: 700 }}>⚠️ Wajib diisi</span>}</div>{currentSurvey.questions?.map((q, i) => (<div key={i} style={{ marginBottom: 20 }}><p style={{ fontWeight: 700, fontSize: 14, marginBottom: 8 }}>{i + 1}. {q.question}</p><div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>{(q.options || []).filter(o => o && o.trim()).map((opt, oi) => (<label key={oi} style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '10px 14px', borderRadius: 10, cursor: 'pointer', background: surveyAnswers[i] === opt ? '#eef2ff' : '#f8fafc', border: `2px solid ${surveyAnswers[i] === opt ? '#3b82f6' : '#e2e8f0'}` }}><input type="radio" name={`sq-${i}`} checked={surveyAnswers[i] === opt} onChange={() => setSurveyAnswers({ ...surveyAnswers, [i]: opt })} style={{ width: 18, height: 18 }} /><span style={{ fontSize: 13 }}>{opt}</span></label>))}</div></div>))}<button onClick={submitSurvey} disabled={surveyLoading} style={{ width: '100%', padding: 14, background: '#3b82f6', color: 'white', border: 'none', borderRadius: 12, fontWeight: 800, fontSize: 15, cursor: surveyLoading ? 'not-allowed' : 'pointer', marginTop: 10, opacity: surveyLoading ? 0.7 : 1 }}>{surveyLoading ? 'Mengirim...' : 'Kirim Jawaban'}</button></div></div>)}
