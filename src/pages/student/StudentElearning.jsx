...
import { 
  ...
  Send, CheckCircle, XCircle, Download, Eye, 
  ExternalLink, FileQuestion, HelpCircle, Search, 
  Layers, Upload, X, Grid3x3, List, 
  File, Hash, Tag, User
} from 'lucide-react';
...
const StudentElearning = () => {
  ...
  const [stats, setStats] = useState({
    totalModules: 0, totalAssignments: 0,
    submittedAssignments: 0, totalQuizzes: 0,
    completedQuizzes: 0, avgScore: 0
  });
  ...
  useEffect(() => {
    ...
    const sName = localStorage.getItem('studentName') || 'Siswa';
    const sNim = localStorage.getItem('studentNim') || localStorage.getItem('studentId') || '';
    ...
    if (sId) {
      getDoc(doc(db, "students", sId)).then(snap => {
        if (snap.exists()) {
          const data = snap.data();
          setStudentKelas(data.kelasSekolah || '');
          setStudentKategori(data.kategori || 'Reguler');
          setStudentNim(data.studentId || data.nim || sNim);
        }
      }).catch(() => {});
    }
  }, []);

  // ===== FETCH MODULES - TANPA DEPENDENCY LOOP =====
  const fetchModules = useCallback(async () => {
    ...
    try {
      ...
      const currentKategori = studentKategori || localStorage.getItem('studentKategori') || 'Reguler';
      const currentKelas = studentKelas || localStorage.getItem('studentKelas') || '';
      ...
      allModules = allModules.filter(module => {
        ...
        const kategoriMatch = targetKategori === "Semua" || targetKategori === currentKategori;
        const kelasMatch = targetKelas === "Semua" || targetKelas === currentKelas;
        return kategoriMatch && kelasMatch;
      });
      
      // Filter siswa spesifik
      const currentNim = studentNim || localStorage.getItem('studentNim') || localStorage.getItem('studentId') || '';
      allModules = allModules.filter(module => {
        if (module.sendToSpecificStudents && module.selectedStudents?.length > 0) {
          return module.selectedStudents.some(s => 
            s.studentId === currentNim || s.id === studentId
          );
        }
        return true;
      });
      ...
      // Fetch submissions pakai studentNim
      if (currentNim) {
        const [snapTugas, snapKuis] = await Promise.all([
          getDocs(query(collection(db, "jawaban_tugas"), where("studentNim", "==", currentNim))),
          getDocs(query(collection(db, "jawaban_kuis"), where("studentNim", "==", currentNim)))
        ]);
        
        const subMap = {};
        snapTugas.forEach(d => { subMap[d.data().modulId] = { id: d.id, ...d.data() }; });
        setSubmissions(subMap);
        
        const quizMap = {};
        snapKuis.forEach(d => { quizMap[d.data().modulId] = { id: d.id, ...d.data() }; });
      ...
      const subCount = allModules.filter(m => submissions[m.id]).length;
      const quizCount = allModules.filter(m => quizSubmissions[m.id]).length;
      
      setStats({
        totalModules: allModules.length,
        totalAssignments: assignments.length,
        submittedAssignments: subCount,
    ...
    setLoading(false);
  }, [studentKategori, studentKelas, studentNim, studentId]);

  // Fetch modules saat data siswa siap
  useEffect(() => {
    fetchModules();
  }, [fetchModules]);
  ...
  const handlePreviewFile = useCallback((url, name) => {
  ...
  const handleSubmitAssignment = useCallback(async () => {
    ...
    try {
      ...
      await addDoc(collection(db, "jawaban_tugas"), {
        ...
        subject: selectedModule.subject || selectedModule.kodeMapel || '',
    ...
    setSubmitting(false);
  }, [selectedAssignment, assignmentAnswer, assignmentFile, selectedModule, studentId, studentName, studentNim, studentKelas, fetchModules]);
  ...
  const renderModuleCard = (module) => {
    ...
    return (
      <div key={module.id} onClick={() => handleModuleClick(module)} style={cardStyles.card}>
        <div style={cardStyles.cover}>
          ...
          {isQuizDone && (
            <span style={{...cardStyles.badgeStatus, background: '#dcfce7', color: '#10b981' }}>✅ Selesai</span>
          )}
          {module.guruId && (
            <span style={cardStyles.idBadge}><Hash size={8} /> {module.guruId}</span>
          )}
        </div>
        <div style={cardStyles.body}>
          ...
          <button style={cardStyles.btn}>Buka <ChevronRight size={14} /></button>
        </div>
      </div>
    );
  };
  ...
  const renderModuleDetail = () => {
    ...
    const coverImage = selectedModule.coverImage || null;
    ...
    return (
      <div style={detailStyles.container}>
        <div style={detailStyles.header}>
          <button onClick={handleBack} style={detailStyles.backBtn}><ChevronLeft size={20} /> Kembali</button>
          <div style={detailStyles.titleSection}>
            <h1 style={detailStyles.title}>{selectedModule.title}</h1>
            <div style={detailStyles.meta}>
              <span style={detailStyles.metaItem}><BookOpen size={14} /> {selectedModule.subject || 'Materi'}</span>
              {selectedModule.kodeMapel && (
                <span style={{...detailStyles.metaItem, background: '#ede9fe', color: '#8b5cf6' }}><Tag size={12} /> {selectedModule.kodeMapel}</span>
              )}
              {selectedModule.guruId && (
                <span style={{...detailStyles.metaItem, background: '#eef2ff', color: '#3b82f6' }}><Hash size={12} /> {selectedModule.guruId}</span>
              )}
              <span style={detailStyles.metaItem}><User size={12} /> {selectedModule.guruName || selectedModule.authorName || 'Guru'}</span>
            </div>
          </div>
        </div>

        {coverImage && (
          <div style={detailStyles.cover}><img src={coverImage} alt={selectedModule.title} style={detailStyles.coverImage} /></div>
        )}

        {selectedModule.description && (
          <div style={detailStyles.desc}><p>{selectedModule.description}</p></div>
        )}

        <div style={detailStyles.tabs}>
          <button onClick={() => setActiveTab('materi')} style={{...detailStyles.tab, background: activeTab === 'materi' ? '#3b82f6' : '#f1f5f9', color: activeTab === 'materi' ? 'white' : '#64748b' }}>
            <BookOpen size={14} /> Materi ({materials.length})
          </button>
          {assignments.length > 0 && (
            <button onClick={() => setActiveTab('tugas')} style={{...detailStyles.tab, background: activeTab === 'tugas' ? '#f59e0b' : '#f1f5f9', color: activeTab === 'tugas' ? 'white' : '#64748b' }}>
              <Send size={14} /> Tugas ({assignments.length})
            </button>
          )}
          {hasQuiz && (
            <button onClick={() => setActiveTab('kuis')} style={{...detailStyles.tab, background: activeTab === 'kuis' ? '#8b5cf6' : '#f1f5f9', color: activeTab === 'kuis' ? 'white' : '#64748b' }}>
          ...
          {activeTab === 'materi' && materials.map((block, idx) => (
            <div key={block.id} style={detailStyles.block}>
              <div style={detailStyles.blockHeader}>
                <span style={detailStyles.blockType}>{block.type === 'text' ? '📄' : block.type === 'file' ? '📁' : block.type === 'video' ? '🎥' : '📝'} {block.type?.toUpperCase() || 'MATERI'}</span>
                <h3 style={detailStyles.blockTitle}>{block.title || `Bagian ${idx + 1}`}</h3>
              </div>
              {block.type === 'text' && <div style={detailStyles.textContent}>{block.content}</div>}
              {block.type === 'file' && block.content && (
                <div style={detailStyles.fileBlock}>
                  <div style={detailStyles.fileInfo}><FileText size={24} color="#3b82f6" /><span>{block.fileName || 'File'}</span></div>
                  <div style={detailStyles.fileActions}>
                    <button onClick={() => handlePreviewFile(block.content, block.fileName)} style={detailStyles.previewBtn}><Eye size={14} /> Preview</button>
                    <a href={block.content} download style={detailStyles.downloadBtn}><Download size={14} /> Unduh</a>
                  </div>
                </div>
              )}
              {block.type === 'video' && block.content && (
                <div style={detailStyles.videoBlock}>
                  {block.content.includes('youtube.com') || block.content.includes('youtu.be') ? (
                    <iframe src={`https://www.youtube.com/embed/${block.content.split('v=')[1]?.split('&')[0] || block.content.split('/').pop()}`} style={detailStyles.videoIframe} allowFullScreen title="Video" />
                  ) : (
                    <a href={block.content} target="_blank" rel="noopener noreferrer" style={detailStyles.linkExternal}><ExternalLink size={16} /> Buka Link Video</a>
                  )}
                </div>
              )}
            </div>
          ...
          {activeTab === 'tugas' && assignments.map(block => {
            ...
            return (
              <div key={block.id} style={{...detailStyles.block, borderLeft: '4px solid #f59e0b' }}>
                ...
                {timeRem && (
                  <div style={{...detailStyles.deadline, color: timeRem.color, background: timeRem.color + '15' }}><Clock size={14} /> {timeRem.text}</div>
                )}
                {isSubmitted ? (
                  <div style={detailStyles.submittedBox}>
                    <CheckCircle size={16} color="#10b981" /><span>Tugas telah dikirim</span>
                    <button onClick={() => { const sub = submissions[selectedModule.id]; if (sub?.fileUrl) handlePreviewFile(sub.fileUrl, sub.fileName); }} style={detailStyles.viewSubmitBtn}><Eye size={14} /> Lihat</button>
                  </div>
                ) : isExpired ? (
                  <div style={{...detailStyles.submittedBox, background: '#fee2e2', color: '#ef4444' }}><XCircle size={16} /> Deadline terlewat</div>
                ) : (
                  <button onClick={() => setSelectedAssignment(block)} style={detailStyles.submitBtn}><Send size={14} /> Kumpulkan Tugas</button>
                )}
              </div>
            );
          })}
          ...
          {activeTab === 'kuis' && hasQuiz && (
            <div style={{...detailStyles.block, borderLeft: '4px solid #8b5cf6' }}>
              ...
              <div style={detailStyles.quizInfo}>
                ...
                {selectedModule.deadlineQuiz && <span>⏰ Deadline: {formatDate(selectedModule.deadlineQuiz)}</span>}
              </div>
              {isQuizDone ? (
                <div style={detailStyles.submittedBox}>
                  <CheckCircle size={16} color="#10b981" /><span>Kuis selesai! Nilai: {quizSub?.score || 0}</span>
                  <button onClick={() => navigate(`/siswa/kuis/${selectedModule.id}`)} style={detailStyles.viewSubmitBtn}><Eye size={14} /> Detail</button>
                </div>
              ) : (
                <button onClick={() => navigate(`/siswa/kuis/${selectedModule.id}`)} style={{...detailStyles.submitBtn, background: '#8b5cf6' }}><HelpCircle size={14} /> Mulai Kuis</button>
              )}
            </div>
          )}
        </div>
        ...
        <div style={modalStyles.content} onClick={e => e.stopPropagation()}>
          ...
          <div style={{ padding: 20 }}>
            ...
            <div style={{ marginBottom: 16 }}>
              ...
              <textarea value={assignmentAnswer} onChange={e => setAssignmentAnswer(e.target.value)} placeholder="Tulis jawaban di sini..." style={{ width: '100%', padding: 10, borderRadius: 8, border: '1px solid #e2e8f0', fontSize: 13, resize: 'vertical', minHeight: 100, fontFamily: 'inherit' }} />
            </div>
            <div>
              ...
              <div style={{ border: '2px dashed #e2e8f0', borderRadius: 8, padding: 20, textAlign: 'center', cursor: 'pointer' }}>
                <input type="file" id="assignmentFile" accept=".pdf,.doc,.docx,.jpg,.jpeg,.png" onChange={e => setAssignmentFile(e.target.files[0])} style={{ display: 'none' }} />
              ...
              {assignmentFile && (
                <button onClick={() => setAssignmentFile(null)} style={{ marginTop: 8, background: '#fee2e2', border: 'none', padding: '4px 12px', borderRadius: 6, fontSize: 11, fontWeight: 600, color: '#ef4444', cursor: 'pointer' }}><X size={12} /> Hapus File</button>
              )}
            </div>
          </div>
  ...
  return (
    <div style={listStyles.container}>
      ...
      <div style={listStyles.header}>
        <div>
          <h1 style={listStyles.title}><BookOpen size={28} color="#652D90" /> E-Learning</h1>
        ...
        <div style={listStyles.viewToggle}>
          <button onClick={() => setViewMode('grid')} style={{...listStyles.viewBtn, background: viewMode === 'grid' ? '#652D90' : '#f1f5f9', color: viewMode === 'grid' ? 'white' : '#64748b' }}><Grid3x3 size={16} /></button>
          <button onClick={() => setViewMode('list')} style={{...listStyles.viewBtn, background: viewMode === 'list' ? '#652D90' : '#f1f5f9', color: viewMode === 'list' ? 'white' : '#64748b' }}><List size={16} /></button>
        </div>
      </div>

      {/* STATS */}
      <div style={listStyles.statsRow}>
        <div style={listStyles.statItem}><span style={listStyles.statValue}>{stats.totalModules}</span><span style={listStyles.statLabel}>📚 Modul</span></div>
        <div style={listStyles.statItem}><span style={listStyles.statValue}>{stats.totalAssignments}</span><span style={listStyles.statLabel}>📝 Tugas</span></div>
        <div style={listStyles.statItem}><span style={listStyles.statValue}>{stats.submittedAssignments}/{stats.totalAssignments}</span><span style={listStyles.statLabel}>✅ Terkirim</span></div>
        <div style={listStyles.statItem}><span style={listStyles.statValue}>{stats.totalQuizzes}</span><span style={listStyles.statLabel}>❓ Kuis</span></div>
      </div>
      ...
      <div style={listStyles.filterBar}>
        <div style={listStyles.searchBox}>
          ...
          <input type="text" placeholder="Cari materi, mapel, atau ID..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} style={listStyles.searchInput} />
          {searchTerm && <button onClick={() => setSearchTerm('')} style={listStyles.clearBtn}>✕</button>}
        </div>
        <div style={listStyles.filterGroup}>
          <select value={filterType} onChange={e => setFilterType(e.target.value)} style={listStyles.filterSelect}>
            <option value="all">Semua</option>
            <option value="module">📚 Modul</option>
            <option value="assignment">📝 Tugas</option>
            <option value="quiz">❓ Kuis</option>
          </select>
          <select value={filterMapel} onChange={e => setFilterMapel(e.target.value)} style={listStyles.filterSelect}>
      ...
      {filteredModules.length === 0 ? (
        <div style={listStyles.emptyState}>
          ...
          <p style={listStyles.emptyDesc}>{searchTerm ? 'Coba ubah kata kunci pencarian' : `Belum ada materi untuk ${studentKategori} - Kelas ${studentKelas}`}</p>
        </div>
      ) : viewMode === 'grid' ? (
        ...
        <div style={listStyles.list}>
          {filteredModules.map(module => {
            ...
            return (
              <div key={module.id} onClick={() => handleModuleClick(module)} style={listStyles.listItem}>
                ...
                <div style={listStyles.listItemContent}>
                  <div style={listStyles.listItemHeader}>
                    ...
                    <span style={{...listStyles.listItemStatus, background: statusInfo.bg, color: statusInfo.color }}>{statusInfo.label}</span>
                  </div>
...
// ============================================================
const cardStyles = {
  card: { background: 'white', borderRadius: 16, overflow: 'hidden', cursor: 'pointer', transition: 'all 0.3s ease', boxShadow: '0 2px 8px rgba(0,0,0,0.04)', border: '1px solid #f1f5f9', height: '100%', display: 'flex', flexDirection: 'column' },
  cover: { position: 'relative', height: 140, background: 'linear-gradient(135deg, #667eea 0%, #764ba2 100%)', overflow: 'hidden', flexShrink: 0 },
  coverImage: { width: '100%', height: '100%', objectFit: 'cover' },
  coverPlaceholder: { width: '100%', height: '100%', display: 'flex', alignItems: 'center', justifyContent: 'center', background: 'linear-gradient(135deg, #667eea 0%, #764ba2 100%)' },
  badgeTop: { position: 'absolute', top: 10, left: 10, display: 'flex', gap: 4, flexWrap: 'wrap' },
  badge: { display: 'inline-flex', alignItems: 'center', gap: 4, padding: '2px 10px', borderRadius: 12, fontSize: 9, fontWeight: 700 },
  badgeStatus: { position: 'absolute', top: 10, right: 10, padding: '2px 10px', borderRadius: 12, fontSize: 9, fontWeight: 700 },
  idBadge: { position: 'absolute', bottom: 10, left: 10, display: 'inline-flex', alignItems: 'center', gap: 3, padding: '1px 8px', borderRadius: 10, fontSize: 8, color: '#3b82f6', background: 'rgba(255,255,255,0.9)', fontWeight: 600 },
  ...
  subject: { fontSize: 11, color: '#64748b', display: 'flex', alignItems: 'center', gap: 4, marginBottom: 8 },
  mapelTag: { fontSize: 8, color: '#8b5cf6', background: '#ede9fe', padding: '1px 6px', borderRadius: 4, display: 'inline-flex', alignItems: 'center', gap: 2 },
  meta: { display: 'flex', gap: 6, flexWrap: 'wrap', marginBottom: 10, marginTop: 'auto' },
  metaItem: { fontSize: 9, padding: '2px 8px', borderRadius: 10, background: '#f1f5f9', color: '#64748b', display: 'inline-flex', alignItems: 'center', gap: 3 },
  btn: { width: '100%', padding: 8, background: '#3b82f6', color: 'white', border: 'none', borderRadius: 8, fontSize: 11, fontWeight: 700, cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 4 }
};

const detailStyles = {
  container: { width: '100%', maxWidth: 900, margin: '0 auto' },
  header: { marginBottom: 24 },
  backBtn: { display: 'inline-flex', alignItems: 'center', gap: 6, background: 'white', border: '1px solid #e2e8f0', padding: '8px 16px', borderRadius: 10, cursor: 'pointer', fontSize: 13, fontWeight: 600, marginBottom: 12 },
  ...
  metaItem: { display: 'inline-flex', alignItems: 'center', gap: 4, fontSize: 11, color: '#64748b', background: '#f1f5f9', padding: '3px 10px', borderRadius: 6 },
  ...
  tab: { flex: 1, padding: '8px 12px', borderRadius: 8, border: 'none', fontWeight: 700, fontSize: 11, cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 4 },
...
const listStyles = {
  container: { width: '100%', padding: '20px', boxSizing: 'border-box', minHeight: '100vh', background: '#f8fafc' },
