...
const ManagePaketHarga = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        if (vaSnap.exists()) {
          setVaInfo(vaSnap.data());
        } else {
          await setDoc(vaRef, vaInfo);
        }
      } catch (error) {
  ...
  const handleAddPaket = (jenjang) => {
    const updated = { ...paketData };
    const newId = `${jenjang.toLowerCase()}_new_${Date.now()}`;
  ...
  // ===== HANDLERS VA =====
  const handleVAChange = (field, value) => {
    setVaInfo(prev => ({ ...prev, [field]: value }));
  };
  ...
  const handleSaveAll = async () => {
    ...
    try {
      ...
      alert("✅ Semua data berhasil disimpan!\n\n• Paket harga: Live di formulir pendaftaran\n• Virtual Account: Siap untuk Midtrans");
    } catch (error) {
    ...
    return (
      <div style={s.wrap}>
        ...
        <div style={s.main(isMobile)}>
          <div style={s.center}><div style={s.spin}></div><p>Memuat data...</p></div>
        </div>
  ...
  return (
    <div style={s.wrap}>
      ...
      <div style={s.main(isMobile)}>
        ...
        <div style={s.header}>
          <div style={s.headerL}>
            <button onClick={() => navigate('/admin/pendaftaran')} style={s.btnBack}>
              <ArrowLeft size={14} /> Pendaftaran
            </button>
        ...
        {/* INFO */}
        <div style={s.infoBox}>
          <span>💡 Harga yang disimpan akan <strong>langsung tampil</strong> di formulir pendaftaran online (iPad/public) dan halaman admin.</span>
        </div>

        <div style={s.grid}>
          {/* ============================================================ */}
          {/* KOLOM KIRI: PAKET HARGA */}
          {/* ============================================================ */}
          <div style={s.col}>
            {Object.keys(paketData).map((jenjang) => (
              <div key={jenjang} style={s.card}>
                <div style={s.cardHead}>
                  ...
                  <button onClick={() => handleAddPaket(jenjang)} style={s.btnAdd}>
                    <Plus size={14} /> Tambah Paket
                  </button>
                </div>
                <div style={s.paketList}>
                  {paketData[jenjang].length === 0 ? (
                    <p style={s.emptyText}>Belum ada paket. Klik "Tambah Paket".</p>
                  ) : (
                      ...
                      <div key={paket.id} style={s.paketItem}>
                        <div style={s.paketInfo}>
                          <input 
                            type="text" 
                            value={paket.nama} 
                            onChange={(e) => handleNamaChange(jenjang, idx, e.target.value)}
                            style={s.paketNama}
                          ...
                          <input 
                            type="text" 
                            value={paket.desc} 
                            onChange={(e) => handleDescChange(jenjang, idx, e.target.value)}
                            style={s.paketDesc}
                        ...
                        <div style={s.paketHarga}>
                          ...
                          <input 
                            ...
                            style={s.hargaInput}
                          />
                          <button onClick={() => handleDeletePaket(jenjang, idx)} style={s.btnDel} title="Hapus">
                            <Trash2 size={14} />
                          </button>
                        </div>
                      </div>
          ...
          <div style={s.col}>
            <div style={s.card}>
              <div style={s.cardHead}>
                ...
                <button onClick={() => setEditingVA(!editingVA)} style={s.btnEdit}>
                  {editingVA ? <X size={14} /> : 'Edit'}
                </button>
              </div>
              ...
              {editingVA ? (
                <div style={s.vaForm}>
                  <div style={s.fg}>
                    <label style={s.lb}>Nama Bank</label>
                    <select value={vaInfo.bankName} onChange={(e) => handleVAChange('bankName', e.target.value)} style={s.inp}>
                  ...
                  <div style={s.fg}>
                    <label style={s.lb}>Nomor Rekening</label>
                    <input type="text" value={vaInfo.accountNumber} onChange={(e) => handleVAChange('accountNumber', e.target.value)} style={s.inp} placeholder="Nomor rekening" />
                  </div>
                  <div style={s.fg}>
                    <label style={s.lb}>Atas Nama</label>
                    <input type="text" value={vaInfo.accountHolder} onChange={(e) => handleVAChange('accountHolder', e.target.value)} style={s.inp} placeholder="Nama pemilik rekening" />
                  </div>
                  <div style={s.fg}>
                    <label style={s.lb}>Instruksi Transfer</label>
                    <textarea value={vaInfo.instructions} onChange={(e) => handleVAChange('instructions', e.target.value)} style={{...s.inp, minHeight: 80, resize: 'vertical'}} placeholder="Instruksi pembayaran" rows={3} />
                  </div>
                </div>
              ) : (
              ...
              <div style={s.vaInfo}>
                <span>💳 Info ini digunakan untuk simulasi pembayaran virtual di menu <strong>"Kirim Virtual"</strong> pada halaman Pendaftaran Online.</span>
              </div>
            </div>
            ...
            <div style={s.card}>
              <div style={s.cardHead}>
                <h3 style={s.cardTitle}>🔌 Integrasi Midtrans</h3>
              </div>
              <div style={s.midInfo}>
                ...
                <p>📂 File: <code>/api/create-payment.js</code></p>
                <p>📘 <a href="https://docs.midtrans.com" target="_blank" rel="noreferrer" style={{color:'#3b82f6'}}>Dokumentasi Midtrans →</a></p>
              </div>
            </div>
          </div>
        </div>
...
const s = {
  ...
  // Midtrans
  midInfo: { fontSize:12, color:'#475569', lineHeight:1.8, '& p': { margin:'0 0 6px' } }
};
