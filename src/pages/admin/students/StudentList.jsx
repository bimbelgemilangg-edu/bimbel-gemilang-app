...
import { Search, Plus, FileText, User, Filter } from 'lucide-react';
    ...
    <div className="p-8 space-y-8 animate-in fade-in">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        ...
        <div className="flex gap-2">
          <button onClick={downloadPDF} className="bg-slate-100 text-slate-600 px-6 py-3 rounded-xl font-black text-xs uppercase hover:bg-slate-200 transition-all flex items-center gap-2">
      ...
      <div className="flex gap-4 bg-white p-2 rounded-2xl shadow-sm border border-slate-100">
        ...
        <select 
          ...
          className="px-6 py-3 bg-slate-50 rounded-xl font-bold text-slate-600 outline-none text-xs uppercase"
        >
      ...
      {/* LIST SISWA (GRID CARD) */}
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {filteredStudents.length === 0 ? (
          <div className="col-span-full py-20 text-center">
            <div className="inline-block p-6 bg-slate-50 rounded-full mb-4 text-slate-300"><User size={48}/></div>
            <h3 className="text-slate-400 font-bold">Tidak ada siswa ditemukan.</h3>
          </div>
        ) : (
          filteredStudents.map((student) => (
            <div 
              ...
              className="bg-white p-6 rounded-3xl border border-slate-100 hover:shadow-xl hover:border-blue-200 transition-all cursor-pointer group relative overflow-hidden"
            >
              ...
              <div className="relative z-10">
                <div className="flex justify-between items-start mb-4">
                  <span className={`px-3 py-1 rounded-full text-[10px] font-black uppercase tracking-widest ${student.schoolLevel === 'SD' ? 'bg-green-100 text-green-600' : student.schoolLevel === 'SMP' ? 'bg-purple-100 text-purple-600' : 'bg-orange-100 text-orange-600'}`}>
                    {student.schoolLevel}
                  </span>
                </div>
                ...
                <div className="border-t border-slate-100 pt-4 flex justify-between items-center">
                  <span className="text-[10px] font-bold text-slate-400 uppercase">Wali: {student.parentName || '-'}</span>
                  <span className="text-[10px] font-black text-blue-600 uppercase">Lihat Detail &rarr;</span>
                </div>
              </div>
            </div>
          ))
