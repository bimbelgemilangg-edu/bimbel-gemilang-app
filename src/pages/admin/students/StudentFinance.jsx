...
const StudentFinance = () => {
  ...
  const [payingIndex, setPayingIndex] = useState(null); // null = lunasin
  ...
  // === VALIDASI ANTI DOUBLE PAYMENT ===
  const isAlreadyLunas = () => {
    if (!student) return false;
    const sisa = (parseInt(student.totalTagihan || 0)) - (parseInt(student.totalBayar || 0));
    return sisa <= 0;
  };

  const isPendaftaranLunas = () => {
    // Jika metodeBayar = Tunai/Transfer dan totalBayar = totalTagihan
  ...
  const openBayarModal = (index = null) => {
    // VALIDASI: jangan bisa bayar kalau pendaftaran sudah lunas
    if (isPendaftaranLunas() && index === null) {
      return showAlert('⚠️ Siswa ini sudah LUNAS saat pendaftaran! Tidak perlu pembayaran lagi.');
    }
    
    setPayingIndex(index);
    if (index !== null && tagihan?.detailCicilan) {
      // VALIDASI: cek apakah cicilan ini sudah lunas
      if (tagihan.detailCicilan[index].status === 'Lunas') {
        return showAlert('⚠️ Cicilan ini sudah dibayar!');
      }
  ...
  const handleBayar = async (e) => {
    ...
    if (sisa <= 0 && payingIndex === null) {
      return showAlert('⚠️ Tidak ada tagihan tersisa. Siswa sudah LUNAS!');
    }
    ...
    try {
      ...
      await updateDoc(doc(db, "students", id), {
        totalBayar: Math.min(newTotalBayar, parseInt(student.totalTagihan || 0)) // jangan lebih dari totalTagihan
      });
    ...
    return (
      <div style={styles.wrapper}>
        ...
        <div style={styles.mainContent(isMobile)}>
          <div style={styles.loadingState}><div style={styles.spinner}></div><p>Memuat data keuangan...</p></div>
        </div>
      </div>
    );
  }
        ...
        <div style={styles.studentCard(isMobile)}>
          ...
          <div style={styles.studentInfo}>
            ...
            <div style={styles.studentMeta}>
              ...
              {isPendaftaranFull && <span style={styles.paidBadge}>✅ Lunas Pendaftaran</span>}
            </div>
          </div>
          <div style={styles.lunasBadge(isLunas)}>
            {isLunas ? <><CheckCircle size={18} /> LUNAS</> : <><Clock size={18} /> BELUM LUNAS</>}
          </div>
        </div>
        ...
        <div style={styles.gridContainer(isMobile)}>
          ...
          <div style={styles.card}>
            ...
            <div style={styles.financeGrid}>
              <div style={styles.financeItem}><TrendingUp size={16} color="#3b82f6" /><div><span style={styles.financeLabel}>Total Tagihan</span><strong style={styles.financeValue}>Rp {(student.totalTagihan || 0).toLocaleString()}</strong></div></div>
              <div style={styles.financeItem}><DollarSign size={16} color="#10b981" /><div><span style={styles.financeLabel}>Sudah Dibayar</span><strong style={{...styles.financeValue, color: '#10b981'}}>Rp {(student.totalBayar || 0).toLocaleString()}</strong></div></div>
              <div style={styles.financeItem}><Receipt size={16} color={sisa > 0 ? '#ef4444' : '#10b981'} /><div><span style={styles.financeLabel}>Sisa Tagihan</span><strong style={{...styles.financeValue, color: sisa > 0 ? '#ef4444' : '#10b981'}}>Rp {sisa.toLocaleString()}</strong></div></div>
              <div style={styles.financeItem}><Calendar size={16} color="#8b5cf6" /><div><span style={styles.financeLabel}>Masa Paket</span><strong style={styles.financeValue}>{student.tanggalMulai || '-'} → {student.tanggalSelesai || '-'}</strong></div></div>
            </div>
            {/* Hanya tampil tombol jika BELUM lunas dan BUKAN pendaftaran lunas */}
            ...
            {isPendaftaranFull && isLunas && (
              <div style={styles.infoBox}><ShieldCheck size={16} color="#10b981" /> Pembayaran sudah selesai saat pendaftaran.</div>
            )}
          </div>
          ...
          <div style={{display: 'flex', flexDirection: 'column', gap: 15}}>
            {tagihan && tagihan.detailCicilan && tagihan.detailCicilan.length > 0 && (
              <div style={styles.card}>
                <h3 style={styles.cardTitle}><CreditCard size={18} /> Detail Cicilan</h3>
                    ...
                    <div key={idx} style={styles.cicilanItem(item.status)}>
                      ...
                      <div style={styles.cicilanRight}>
                        ...
                        {item.status === 'Lunas' ? (
                          ...
                          <span style={styles.badgeLunas}>✅ Lunas (Pendaftaran)</span>
                        ) : (
            ...
            <div style={styles.card}>
              ...
              {financeLogs.length === 0 ? (
                <div style={styles.emptyHistory}><AlertCircle size={32} color="#94a3b8" /><p>Belum ada riwayat pembayaran</p></div>
              ) : (
                  ...
                  {financeLogs.slice(0, 10).map((log) => (
                    <div key={log.id} style={styles.historyItem}>
                      ...
                      <div style={styles.historyRight}>
                        ...
                        <span style={styles.historyMethod(log.method)}>{log.method === 'Tunai' ? '💵 Tunai' : '💳 Transfer'}</span>
                      </div>
                    </div>
                  ))}
                </div>
          ...
          <div style={styles.modalOverlay} onClick={() => setShowPayModal(false)}>
            <div style={styles.modalContent(isMobile)} onClick={e => e.stopPropagation()}>
              <div style={styles.modalHeader}>
                <h3 style={styles.modalTitle}><CreditCard size={20} />{payingIndex !== null ? `Bayar Cicilan ke-${payingIndex + 1}` : 'Pembayaran Lunas'}</h3>
                <button onClick={() => setShowPayModal(false)} style={styles.modalClose}><X size={18} /></button>
              </div>
              <div style={styles.modalBody}>
                <div style={styles.payInfo}><span>Sisa Tagihan</span><strong style={{color: '#ef4444'}}>Rp {sisa.toLocaleString()}</strong></div>
                {payingIndex !== null && tagihan?.detailCicilan && (
                  <div style={styles.payInfo}><span>Nominal Cicilan</span><strong>Rp {tagihan.detailCicilan[payingIndex].nominal.toLocaleString()}</strong></div>
                )}
                <form onSubmit={handleBayar}>
                  <div style={styles.inputGroup}><label style={styles.label}>Jumlah Bayar (Rp)</label><input type="number" style={styles.input} value={payAmount} onChange={e => setPayAmount(e.target.value)} required autoFocus /></div>
                  ...
                  <div style={styles.modalFooter}>
                    ...
                    <button type="submit" style={styles.btnSave} disabled={isProcessing}>{isProcessing ? '⏳ Memproses...' : <><Save size={16} /> Konfirmasi Bayar</>}</button>
                  </div>
                </form>
              </div>
            </div>
...
const styles = {
  ...
  paidBadge: { padding: '3px 10px', borderRadius: 10, fontSize: 10, fontWeight: 'bold', background: '#dcfce7', color: '#166534', display: 'flex', alignItems: 'center', gap: 4 },
  lunasBadge: (isLunas) => ({ display: 'flex', alignItems: 'center', gap: 6, padding: '8px 16px', borderRadius: 12, fontSize: 14, fontWeight: 'bold', background: isLunas ? '#dcfce7' : '#fee2e2', color: isLunas ? '#166534' : '#ef4444', flexShrink: 0 }),
  ...
  infoBox: { display: 'flex', alignItems: 'center', gap: 8, marginTop: 16, padding: '14px', background: '#f0fdf4', borderRadius: 10, border: '1px solid #bbf7d0', fontSize: 13, color: '#166534', fontWeight: 'bold' },
  btnLunasin: { width: '100%', marginTop: 16, padding: '14px', background: '#10b981', color: 'white', border: 'none', borderRadius: 10, fontWeight: 'bold', fontSize: 14, cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8, boxShadow: '0 4px 12px rgba(16,185,129,0.3)' },
  cicilanList: { display: 'flex', flexDirection: 'column', gap: 8 },
  cicilanItem: (status) => ({ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: 12, borderRadius: 10, background: status === 'Lunas' ? '#f0fdf4' : '#fefce8', border: `1px solid ${status === 'Lunas' ? '#bbf7d0' : '#fef08a'}`, flexWrap: 'wrap', gap: 8 }),
  ...
  badgeLunas: { padding: '4px 10px', borderRadius: 8, fontSize: 10, fontWeight: 'bold', background: '#dcfce7', color: '#166534' },
  btnBayarCicilan: { padding: '6px 14px', borderRadius: 8, background: '#f59e0b', color: 'white', border: 'none', fontWeight: 'bold', fontSize: 11, cursor: 'pointer' },
  ...
  historyItem: { display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '10px 12px', borderRadius: 8, background: '#f8fafc' },
  historyLeft: { display: 'flex', alignItems: 'center', gap: 10 },
  historyDot: (method) => ({ width: 8, height: 8, borderRadius: '50%', background: method === 'Tunai' ? '#f59e0b' : '#3b82f6' }),
  ...
  historyMethod: (method) => ({ fontSize: 9, padding: '1px 6px', borderRadius: 4, background: method === 'Tunai' ? '#fef3c7' : '#e0e7ff', color: method === 'Tunai' ? '#b45309' : '#3730a3' }),
