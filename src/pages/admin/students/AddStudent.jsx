  ...
  const generateStudentId = async () => {
    try {
      ...
      if (!snap.empty) {
        ...
        if (lastId && lastId.startsWith(`STD-${year}`)) {
    ...
    <div style={styles.stepContent}>
      ...
      <div style={styles.infoBox}>
        <div style={styles.infoRow}>
          <span>📅 Mulai:</span>
          <strong>{formData.tanggalMulai}</strong>
        </div>
        <div style={styles.infoRow}>
          <span>🏁 Selesai:</span>
          <strong style={{color: '#ef4444'}}>{getTanggalSelesai()}</strong>
        </div>
        <div style={styles.infoRow}>
          <span>⏳ Durasi:</span>
          <strong>{formData.durasiBulan} bulan</strong>
        </div>
      </div>
      ...
      <div style={styles.accountPreview}>
        <div style={styles.accountHeader}>
          <IdCard size={16} />
          <span>Akun Portal (Auto-generated)</span>
        </div>
        <div style={styles.accountRow}>
          <span style={styles.accountLabel}>Username:</span>
          <code style={styles.accountValue}>{getUsername()}</code>
        </div>
        <div style={styles.accountRow}>
          <span style={styles.accountLabel}>Password:</span>
          <code style={styles.accountValue}>{getPassword()}</code>
        </div>
      </div>
    </div>
  );
    ...
    <div style={styles.stepContent}>
      ...
      <div style={styles.paymentCard}>
        <div style={styles.paymentRow}>
          ...
          <span>Rp {getBasePrice().toLocaleString()}</span>
        </div>
        <div style={styles.paymentRow}>
          <span>
            <label style={{cursor: 'pointer', display: 'flex', alignItems: 'center', gap: 6}}>
              <input 
                type="checkbox" 
                checked={formData.biayaDaftar} 
                onChange={e => updateField('biayaDaftar', e.target.checked)} 
              />
              Biaya Pendaftaran
            </label>
          </span>
      ...
      <div style={styles.inputGroup}>
        ...
        <div style={styles.tabRow3}>
          <button 
            type="button"
            onClick={() => updateField('metodeBayar', 'Tunai')}
            style={styles.methodBtn(formData.metodeBayar === 'Tunai')}
          >
            💵 Tunai
          </button>
          <button 
            type="button"
            onClick={() => updateField('metodeBayar', 'Transfer')}
            style={styles.methodBtn(formData.metodeBayar === 'Transfer')}
          >
            💳 Transfer
          </button>
          <button 
            type="button"
            onClick={() => updateField('metodeBayar', 'Cicilan')}
            style={styles.methodBtn(formData.metodeBayar === 'Cicilan')}
          >
            📋 Cicilan
          </button>
        </div>
        ...
        <div style={styles.cicilanBox}>
          ...
          <div style={styles.tenorRow}>
            {[1,2,3,4,5,6].map(t => (
              <button 
                key={t}
                type="button"
                onClick={() => updateField('tenor', t)}
                style={styles.tenorBtn(formData.tenor === t)}
              >
                {t}x
              </button>
            ))}
          ...
          <div style={styles.inputGroup}>
            ...
            <input 
              type="date" 
              style={styles.input} 
              value={formData.tanggalCicilan1} 
              onChange={e => updateField('tanggalCicilan1', e.target.value)} 
            />
          </div>

          <div style={styles.cicilanPreview}>
            <p style={{fontWeight: 'bold', marginBottom: 8}}>
              {formData.tenor}x Cicilan @ Rp {hitungCicilan().toLocaleString()}
            </p>
      ...
      <div style={styles.mainContent(isMobile)}>
        ...
        <div style={styles.header(isMobile)}>
          ...
          <h2 style={styles.pageTitle(isMobile)}>
            <Sparkles size={20} color="#3b82f6" /> Pendaftaran Siswa Baru
          </h2>
        </div>
        ...
        <div style={styles.card}>
          ...
          <div style={styles.navButtons}>
            ...
            {step < totalSteps ? (
              <button onClick={() => setStep(step + 1)} style={styles.btnNext}>
...
const styles = {
  ...
  stepLabel: (active) => ({ 
    fontSize: 11, fontWeight: active ? 'bold' : '500',
  ...
  accountHeader: { 
    ...
    fontWeight: 'bold', fontSize: 12, color: '#854d0e', marginBottom: 8 
  },
  ...
  paymentTotal: { 
    ...
    marginTop: 8, borderTop: '2px solid #bbf7d0',
