...
const AddStudent = () => {
    ...
    const fetchPricing = async () => {
      try {
        ...
        if (docSnap.exists() && docSnap.data().prices) {
          ...
          // Merge dengan default kosong agar tidak error
          setPricing({
    ...
    try {
      ...
      if (!snap.empty) {
        ...
        if (lastId) {
          ...
          if (match && match[1] === year.toString()) nextNumber = parseInt(match[2]) + 1;
        }
      }
  ...
  useEffect(() => {
    if (!tglLahir.tahun) setTglLahir(prev => ({ ...prev, tahun: (currentYear - 12).toString() }));
  }, []);

  // === FORM DATA ===
  const [formData, setFormData] = useState({
    nama: '', tempatLahir: '', alamat: '', noHp: '',
    namaAyah: '', namaIbu: '',
    kelasSekolah: '1 SD',
    programType: 'Reguler', jenjang: 'SD', paket: 'paket1',
    englishLevel: 'kids',
    tanggalMulai: new Date().toISOString().split('T')[0],
    durasiBulan: 3, metodeBayar: 'Tunai',
    biayaDaftar: true, diskon: 0, tenor: 1,
  ...
  const getUsername = () => {
    ...
    const base = formData.nama.split(' ')[0].toLowerCase().replace(/[^a-z0-9]/g, '');
    return `${base}${Math.floor(100 + Math.random() * 900)}@gemilang.com`;
  };

  const getPassword = () => {
    if (!formData.nama) return '';
    const base = formData.nama.split(' ')[0].toLowerCase().replace(/[^a-z0-9]/g, '');
    return tglLahir.tahun ? `${base}${tglLahir.tahun}` : `${base}123`;
  };
  ...
  useEffect(() => {
    if (formData.metodeBayar === 'Cicilan' && formData.tenor > 0) {
      const dates = [];
      const start = new Date(formData.tanggalCicilan1);
  ...
  // === HARGA (OTOMATIS DARI PAKET YANG ADA) ===
  const getBasePrice = () => {
    if (formData.programType === 'English') {
      return parseInt(pricing.english?.[formData.englishLevel] || 0);
    }
    const level = formData.jenjang.toLowerCase();
    return parseInt(pricing[level]?.[formData.paket] || 0);
  };
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const studentData = {
        studentId, nama: formData.nama,
        username: getUsername().toLowerCase(), password: getPassword(),
        role: 'siswa', kategori: formData.programType,
        ...
        tempatLahir: formData.tempatLahir, tanggalLahir: tanggalLahirStr,
        ortu: { ayah: formData.namaAyah, ibu: formData.namaIbu, alamat: formData.alamat, hp: formData.noHp },
        tanggalMulai: formData.tanggalMulai, tanggalSelesai,
      ...
      if (formData.metodeBayar === 'Tunai' || formData.metodeBayar === 'Transfer') {
        await addDoc(collection(db, "finance_logs"), {
          studentId, namaSiswa: formData.nama, date: today,
          type: 'Pemasukan', category: 'Pendaftaran', amount: totalTagihan,
          method: formData.metodeBayar,
          note: `Pendaftaran Baru: ${formData.nama} - LUNAS`,
          createdAt: serverTimestamp()
        });
        ...
        const installments = formData.customDueDates.map((dateStr, index) => ({
          bulanKe: index + 1, nominal: cicilanNominal,
          status: 'Belum Lunas', jatuhTempo: dateStr, tanggalBayar: null
        }));
        await addDoc(collection(db, "finance_tagihan"), {
          studentId, namaSiswa: formData.nama, noHp: formData.noHp,
          totalTagihan, sisaTagihan: totalTagihan,
          detailCicilan: installments, createdAt: serverTimestamp()
        });
      }
  ...
  const renderStep1 = () => (
    <div style={styles.stepContent}>
      <div style={styles.sectionHeader}><User size={20} color="#3b82f6" /><h3 style={styles.sectionTitle}>Data Diri Siswa</h3></div>
      <div style={styles.inputGroup}><label style={styles.label}>Nama Lengkap *</label><input style={styles.input} placeholder="Nama lengkap siswa" value={formData.nama} onChange={e => updateField('nama', e.target.value)} required /></div>
      <div style={styles.inputGroup}>
        <label style={styles.label}><Calendar size={12} style={{marginRight:4}} /> Tanggal Lahir</label>
        <div style={styles.tglLahirRow}>
          <select style={{...styles.select,flex:1}} value={tglLahir.hari} onChange={e=>setTglLahir(p=>({...p,hari:e.target.value}))}><option value="">Tgl</option>{hariOptions.map(h=><option key={h} value={h}>{h}</option>)}</select>
          <select style={{...styles.select,flex:2}} value={tglLahir.bulan} onChange={e=>setTglLahir(p=>({...p,bulan:e.target.value}))}><option value="">Bulan</option>{bulanOptions.map(b=><option key={b.value} value={b.value}>{b.label}</option>)}</select>
          <select style={{...styles.select,flex:1.5}} value={tglLahir.tahun} onChange={e=>setTglLahir(p=>({...p,tahun:e.target.value}))}><option value="">Tahun</option>{tahunOptions.map(y=><option key={y} value={y}>{y}</option>)}</select>
        </div>
        {getTanggalLahirStr() && <div style={styles.tglPreview}>📅 {getTanggalLahirStr()}</div>}
      </div>
      <div style={styles.row2}>
        <div style={styles.inputGroup}><label style={styles.label}>Tempat Lahir</label><input style={styles.input} placeholder="Kota" value={formData.tempatLahir} onChange={e=>updateField('tempatLahir',e.target.value)} /></div>
      ...
      <div style={styles.sectionHeader}><Phone size={20} color="#3b82f6" /><h3 style={styles.sectionTitle}>Kontak & Orang Tua</h3></div>
      <div style={styles.inputGroup}><label style={styles.label}>No HP / WhatsApp *</label><input type="tel" style={styles.input} placeholder="628xxx" value={formData.noHp} onChange={e=>updateField('noHp',e.target.value)} required /></div>
      <div style={styles.inputGroup}><label style={styles.label}>Alamat</label><textarea style={styles.textarea} placeholder="Alamat lengkap..." value={formData.alamat} onChange={e=>updateField('alamat',e.target.value)} rows={2} /></div>
      <div style={styles.row2}><div style={styles.inputGroup}><label style={styles.label}>Nama Ayah</label><input style={styles.input} placeholder="Nama ayah" value={formData.namaAyah} onChange={e=>updateField('namaAyah',e.target.value)} /></div><div style={styles.inputGroup}><label style={styles.label}>Nama Ibu</label><input style={styles.input} placeholder="Nama ibu" value={formData.namaIbu} onChange={e=>updateField('namaIbu',e.target.value)} /></div></div>
    </div>
  );

  const renderStep2 = () => (
    <div style={styles.stepContent}>
      <div style={styles.sectionHeader}><BookOpen size={20} color="#8b5cf6" /><h3 style={styles.sectionTitle}>Program & Paket Belajar</h3></div>
      <div style={styles.inputGroup}>
      ...
      <div style={styles.sectionHeader}><Calendar size={20} color="#8b5cf6" /><h3 style={styles.sectionTitle}>Masa Aktif Paket</h3></div>
      <div style={styles.row2}>
        <div style={styles.inputGroup}><label style={styles.label}>Tanggal Mulai</label><input type="date" style={styles.input} value={formData.tanggalMulai} onChange={e=>updateField('tanggalMulai',e.target.value)} /></div>
        <div style={styles.inputGroup}><label style={styles.label}>Durasi</label><select style={styles.input} value={formData.durasiBulan} onChange={e=>updateField('durasiBulan',e.target.value)}><option value={1}>1 Bulan</option><option value={3}>3 Bulan</option><option value={6}>6 Bulan</option><option value={12}>12 Bulan</option></select></div>
      </div>
      <div style={styles.infoBox}><div style={styles.infoRow}><span>📅 Mulai:</span><strong>{formData.tanggalMulai}</strong></div><div style={styles.infoRow}><span>🏁 Selesai:</span><strong style={{color:'#ef4444'}}>{getTanggalSelesai()}</strong></div><div style={styles.infoRow}><span>⏳ Durasi:</span><strong>{formData.durasiBulan} bulan</strong></div></div>
      <div style={styles.accountPreview}><div style={styles.accountHeader}><IdCard size={16} /><span>Akun Portal (Auto-generated)</span></div><div style={styles.accountRow}><span style={styles.accountLabel}>Username:</span><code style={styles.accountValue}>{getUsername()}</code></div><div style={styles.accountRow}><span style={styles.accountLabel}>Password:</span><code style={styles.accountValue}>{getPassword()}</code></div></div>
    </div>
  );

  const renderStep3 = () => (
    <div style={styles.stepContent}>
      <div style={styles.sectionHeader}><CreditCard size={20} color="#10b981" /><h3 style={styles.sectionTitle}>Pembayaran</h3></div>
      <div style={styles.paymentCard}>
        <div style={styles.paymentRow}><span>Biaya Paket</span><span>Rp {getBasePrice().toLocaleString()}</span></div>
        <div style={styles.paymentRow}><span><label style={{cursor:'pointer',display:'flex',alignItems:'center',gap:6}}><input type="checkbox" checked={formData.biayaDaftar} onChange={e=>updateField('biayaDaftar',e.target.checked)} />Biaya Pendaftaran</label></span><span>Rp 25.000</span></div>
      ...
      <div style={styles.inputGroup}><label style={styles.label}>Metode Pembayaran</label><div style={styles.tabRow3}><button type="button" onClick={()=>updateField('metodeBayar','Tunai')} style={styles.methodBtn(formData.metodeBayar==='Tunai')}>💵 Tunai</button><button type="button" onClick={()=>updateField('metodeBayar','Transfer')} style={styles.methodBtn(formData.metodeBayar==='Transfer')}>💳 Transfer</button><button type="button" onClick={()=>updateField('metodeBayar','Cicilan')} style={styles.methodBtn(formData.metodeBayar==='Cicilan')}>📋 Cicilan</button></div></div>
      {formData.metodeBayar==='Cicilan' && (
        <div style={styles.cicilanBox}>
          <label style={styles.label}>Tenor Cicilan</label><div style={styles.tenorRow}>{[1,2,3,4,5,6].map(t=><button key={t} type="button" onClick={()=>updateField('tenor',t)} style={styles.tenorBtn(formData.tenor===t)}>{t}x</button>)}</div>
          <div style={styles.inputGroup}><label style={styles.label}>Tanggal Cicilan Pertama</label><input type="date" style={styles.input} value={formData.tanggalCicilan1} onChange={e=>updateField('tanggalCicilan1',e.target.value)} /></div>
          <div style={styles.cicilanPreview}><p style={{fontWeight:'bold',marginBottom:8}}>{formData.tenor}x Cicilan @ Rp {hitungCicilan().toLocaleString()}</p>{formData.customDueDates.map((date,idx)=><div key={idx} style={styles.cicilanItem}><span>Cicilan ke-{idx+1}</span><span>{date}</span></div>)}</div>
        </div>
      )}
    </div>
  );
  ...
  return (
    <div style={styles.wrapper}>
      ...
      <div style={styles.mainContent(isMobile)}>
        ...
        <div style={styles.header(isMobile)}><button onClick={()=>navigate('/admin/students')} style={styles.backBtn}><ArrowLeft size={16} /> Kembali</button><h2 style={styles.pageTitle(isMobile)}><Sparkles size={20} color="#3b82f6" /> Pendaftaran Siswa Baru</h2></div>
        <div style={styles.stepIndicator(isMobile)}>{stepLabels.map((label,idx)=>(<div key={idx} style={styles.stepItem}><div style={styles.stepCircle(idx+1===step,idx+1<step)}>{idx+1<step?<CheckCircle size={16}/>:idx+1}</div><span style={styles.stepLabel(idx+1===step)}>{label}</span>{idx<totalSteps-1&&<div style={styles.stepLine}/>}</div>))}</div>
        <div style={styles.card}>
          {step===1&&renderStep1()}{step===2&&renderStep2()}{step===3&&renderStep3()}
          <div style={styles.navButtons}>
            {step>1&&<button onClick={()=>setStep(step-1)} style={styles.btnPrev}><ChevronLeft size={16}/> Sebelumnya</button>}
            {step<totalSteps?<button onClick={()=>setStep(step+1)} style={styles.btnNext}>Selanjutnya <ChevronRight size={16}/></button>:<button onClick={handleSubmit} style={styles.btnSave} disabled={loading}>{loading?'⏳ Menyimpan...':<><Save size={16}/> Simpan & Selesai</>}</button>}
          </div>
        </div>
      </div>
...
const styles = {
  ...
  stepCircle:(a,d)=>({width:32,height:32,borderRadius:'50%',display:'flex',alignItems:'center',justifyContent:'center',fontSize:13,fontWeight:'bold',background:d?'#10b981':a?'#3b82f6':'#e2e8f0',color:d||a?'white':'#94a3b8',flexShrink:0}),
  stepLabel:(a)=>({fontSize:11,fontWeight:a?'bold':'500',color:a?'#1e293b':'#94a3b8',marginLeft:6,marginRight:6,whiteSpace:'nowrap'}),
  ...
  tabBtn:(a,c='#3b82f6')=>({flex:1,padding:'10px 16px',borderRadius:10,border:a?`2px solid ${c}`:'1px solid #e2e8f0',background:a?'#eff6ff':'white',color:a?c:'#64748b',fontWeight:a?'bold':'500',fontSize:13,cursor:'pointer',transition:'0.2s'}),
  ...
  methodBtn:(a)=>({flex:1,padding:'10px',borderRadius:10,border:a?'2px solid #10b981':'1px solid #e2e8f0',background:a?'#f0fdf4':'white',color:a?'#166534':'#64748b',fontWeight:a?'bold':'500',fontSize:12,cursor:'pointer',transition:'0.2s'}),
  ...
  tenorBtn:(a)=>({padding:'8px 16px',borderRadius:8,border:a?'2px solid #f59e0b':'1px solid #e2e8f0',background:a?'#fffbeb':'white',color:a?'#b45309':'#64748b',fontWeight:a?'bold':'500',fontSize:13,cursor:'pointer'}),
