...
export default function StudentForm({ db, onCancel, onSuccess, initialData }) {
  const isEditing = !!initialData; // Cek apakah ini mode Edit atau Baru
  ...
  const [formData, setFormData] = useState({
    ...
    fatherName: "", fatherJob: "", // Field Baru
    motherName: "", motherJob: "", // Field Baru
    emergencyWAPhone: "", 
    packageDuration: "1"
  });

  // --- STATE KEUANGAN (Khusus Pendaftaran Baru) ---
  ...
  // Load Harga & Data Lama (Jika Edit)
  useEffect(() => {
    getDoc(doc(db, "settings", "prices")).then(s => s.exists() && setPrices(s.data()));
    if(initialData) setFormData(initialData);
  }, [db, initialData]);

  // Generate Tanggal Cicilan Otomatis
  useEffect(() => {
    const dates = [];
    const today = new Date();
    for (let i = 0; i < installmentMonths; i++) {
      const d = new Date(today);
      d.setMonth(today.getMonth() + i + 1); // Bulan depan
      dates.push(d.toISOString().split('T')[0]);
    }
    setDueDates(dates);
  }, [installmentMonths]);

  // Hitung Keuangan Realtime
  const calculateFinancials = () => {
    ...
    // Jika Lunas, DP = Total. Jika Cicilan, DP = Inputan User.
    ...
    try {
      ...
      // --- KASUS 2: PENDAFTARAN BARU (+ KEUANGAN) ---
        ...
        await runTransaction(db, async (t) => {
          ...
          // B. Simpan Pembayaran (Uang Masuk / DP)
          if (fin.paidNow > 0) {
          ...
          if (fin.remaining > 0 && paymentType === 'Cicilan') {
            ...
            for (let i = 0; i < installmentMonths; i++) {
              // Hitung sisa agar pas di bulan terakhir
              ...
              if (currentAmount > 0) {
                  ...
                  t.set(invRef, { 
                    ...
                    status: 'unpaid', 
                    dueDate: dueDates[i], // Tanggal Jatuh Tempo sesuai inputan
  ...
  return (
    <div className="bg-white rounded-[3rem] shadow-2xl border border-slate-200 overflow-hidden animate-in zoom-in duration-300">
      <div className="bg-blue-600 p-8 flex justify-between items-center text-white">
        <div><h2 className="text-3xl font-black uppercase">{isEditing ? 'Edit Data Siswa' : 'Pendaftaran Siswa'}</h2><p className="text-xs font-bold uppercase mt-1">Langkah {step} dari 3</p></div>
      ...
      <form onSubmit={handleSave} className="p-10 space-y-6">
        ...
        {/* STEP 2: ORANG TUA (LENGKAP DENGAN PEKERJAAN) */}
        {step === 2 && (
          <div className="grid md:grid-cols-2 gap-6 animate-in fade-in">
        ...
        {/* STEP 3: KEUANGAN (DISKON & CICILAN) - HANYA MUNCUL SAAT DAFTAR BARU */}
        {step === 3 && (
          <div className="space-y-8 animate-in fade-in">
            {isEditing ? (
               <div className="text-center py-10 bg-slate-50 rounded-3xl border border-dashed border-slate-300"><p className="font-bold text-slate-400 uppercase">Edit Keuangan Hanya via Menu Finance Pusat.</p></div>
            ) : (
              <div className="grid md:grid-cols-2 gap-8">
                ...
                <div className="space-y-4">
                  ...
                  <div className="bg-slate-50 p-6 rounded-3xl border">
                    ...
                    {paymentType === 'Cicilan' && <div><label className="lbl">Tenor & Jatuh Tempo</label><select className="inp mb-2" value={installmentMonths} onChange={e=>setInstallmentMonths(parseInt(e.target.value))}><option value="1">1 Bulan</option><option value="2">2 Bulan</option><option value="3">3 Bulan</option></select>{dueDates.slice(0,installmentMonths).map((d,i)=><div key={i} className="flex gap-2 mt-2"><span className="text-[10px] w-10 font-bold mt-2">Ke-{i+1}</span><input type="date" className="inp py-1 text-xs" value={d} onChange={e=>handleDateChange(i,e.target.value)}/></div>)}</div>}
                  </div>
                </div>
                {/* KANAN: Ringkasan */}
                <div className="flex flex-col justify-between">
                  <div className="bg-slate-900 text-white p-6 rounded-[2rem]">
                    ...
                    {paymentType === 'Cicilan' && <div className="bg-white p-3 rounded-xl"><label className="text-[10px] text-black font-bold block mb-1">Bayar DP Sekarang</label><input className="w-full text-2xl font-black text-blue-600 outline-none" placeholder="0" value={dpAmountStr} onChange={e=>setDpAmountStr(formatRupiah(e.target.value.replace(/\D/g,"")))}/></div>}
                  </div>
                </div>
              </div>
            )}
            <div className="flex justify-between mt-4"><button type="button" onClick={()=>setStep(2)} className="btn-back"><ArrowLeft/></button><button type="submit" className="bg-blue-600 text-white px-8 py-4 rounded-2xl font-black uppercase shadow-xl flex gap-2"><Save size={20}/> {isEditing ? 'Simpan Data' : 'Proses Daftar'}</button></div>
          </div>
        )}
      </form>
