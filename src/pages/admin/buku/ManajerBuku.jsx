...
export default function ManajerBuku() {
  ...
  const [modeFormBuku, setModeFormBuku] = useState(null);
  ...
  const [aiJalan, setAiJalan] = useState(null);
  const [aiPesan, setAiPesan] = useState('');
  const [potong, setPotong] = useState(null);
  ...
  const simpanDariEditor = async (babBaru) => {
    ...
    const payload = {
      id: babId,
      judul: babBaru.judul || babLama?.judul || 'Bab',
      urutan: babLama?.urutan ?? babList.length,
      tipe: 'terstruktur',
      sections: sanitasiFirestore(babBaru.sections || []),
      ujiPemahaman: sanitasiFirestore(babBaru.ujiPemahaman || []),
      sumber: 'editor',
      updatedAt: Date.now(),
    };
    ...
    <div style={st.page}>
      ...
      {bukuDipilih && (
        <div style={st.panel}>
          ...
          {babList.map((bab, i) => {
            const mode = modeBabSekarang(bab);
            return (
          ...
          {modeBab && (
            <div style={{ marginTop: 12 }}>
                  ...
                  <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                    {images.map((img, idx) => (
                      <div key={idx} style={st.imgRow}>
                        ...
                        <button onClick={() => salin(img.url, 'URL')} style={st.btnTiny}><Copy size={12} /> URL</button>
                        <button onClick={() => salin(snippetVisual(img.url), 'Snippet visual soal')} style={st.btnTiny}><FileCode size={12} /> Soal</button>
                        <button onClick={() => salin(snippetBlok(img.url), 'Snippet blok materi')} style={st.btnTiny}><Layers size={12} /> Blok</button>
                        <button onClick={() => setImages((prev) => prev.filter((_, i) => i !== idx))} style={{ ...st.iconBtn, color: '#dc2626' }}><X size={13} /></button>
                      </div>
                    ))}
                  </div>
                )}
              ...
              <div style={{ display: 'flex', gap: 8, marginTop: 10 }}>
                <button onClick={simpanBab} style={st.btnPrimary}><Save size={14} /> Validasi & Simpan</button>
      ...
      {editBab && bukuDipilih && (
        <EditorBab
          ...
          bab={editBab.bab}
          indexBab={null}
          sumberPdf={editBab.bab?.pdfUrl || ''}
          halMulai={editBab.bab?.halamanMulai || 1}
...
const st = {
  ...
  babRow: { display: 'flex', alignItems: 'center', gap: 10, padding: '8px 10px', borderRadius: 10, border: '1px solid #eef2ff', background: '#f8fafc', marginBottom: 6, flexWrap: 'wrap' },
