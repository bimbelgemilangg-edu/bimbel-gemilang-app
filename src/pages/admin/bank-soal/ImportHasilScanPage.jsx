...
//
// 🔥 PERBAIKAN RONDE INI ("agar soal yang di-upload rapi"):
// Logika inti (Leitner, XP, streak, jatah harian, filter jenjang/kelas/
// mapel) TIDAK DISENTUH. Yang diperbaiki adalah LAPISAN RENDER soal
// hasil upload/import, karena bentuk field di Bank Soal ternyata tidak
// seragam antar sumber upload:
//   - teks soal bisa di `soal`, `teks_soal`, `teksSoal`, `pertanyaan`;
//   - bacaan bisa STRING polos maupun objek {teks, gambar};
//   - gambar bisa array string data-URL, array objek {dataUrl/url/
//     uploadedUrl}, atau field `gambarUrls`;
//   - tabel bisa menempel sebagai HTML mentah di `tabel_soal` atau
//     bahkan MENYUSUP di dalam teks soal/pilihan jawaban;
//   - pilihan jawaban bisa string polos, string berisi <table>/<img>,
//     atau objek {teks, gambar}.
// Sebelumnya campuran ini dirender apa adanya sehingga soal upload
// kelihatan berantakan (tag HTML mentah kelihatan, tabel tanpa garis,
// gambar kegedean, baris baru mati). Sekarang ada satu lapis
// NORMALISASI (normalisasiSoal()) + komponen render kaya
// (KontenKaya/GambarSoal) yang menata semuanya: tabel jadi tabel
// bergaris yang bisa di-scroll horizontal di layar sempit, gambar
// dibatasi ukurannya & rata tengah, baris baru dihormati, dan LaTeX
// gaya $...$ MAUPUN \(...\) tetap dirender di bagian teksnya.
// ============================================================
...
// ============================================================
// 🔥🔥 LAPISAN PERAPIHAN SOAL UPLOAD (RONDE INI)
// ============================================================

// CSS dasar buat konten hasil upload -- dipasang sekali per layar lewat
// <StyleDasar/>. Dipakai kelas (bukan style inline) supaya elemen yang
// LAHIR dari dangerouslySetInnerHTML (tabel/gambar di dalam HTML soal)
// ikut tertata juga -- style inline tidak bisa menjangkau anak-anak
// hasil inject HTML.
const CSS_DASAR = `
.gm-teks { white-space: pre-wrap; line-height: 1.65; word-wrap: break-word; }
.gm-teks p, .gm-html p { margin: 4px 0; }
.gm-wrap-table { overflow-x: auto; -webkit-overflow-scrolling: touch; }
.gm-html table, table.gm-table { border-collapse: collapse; width: 100%; font-size: 12.5px; margin: 8px 0; background: #fff; }
.gm-html th, .gm-html td, table.gm-table th, table.gm-table td { border: 1px solid #d7dfe8; padding: 6px 8px; text-align: left; vertical-align: top; line-height: 1.45; }
.gm-html th, table.gm-table th { background: #f1f5f9; font-weight: 700; }
.gm-html img, img.gm-img { max-width: 100%; height: auto; border-radius: 10px; border: 1px solid #e2e8f0; background: #fff; display: block; }
.gm-html ol, .gm-html ul { margin: 4px 0 4px 18px; padding: 0; }
.gm-chip { display: inline-flex; align-items: center; gap: 4px; background: #f1f0f9; color: #4C1D95; border-radius: 999px; padding: 2px 9px; font-size: 10.5px; font-weight: 700; }
`;
function StyleDasar() {
  return <style>{CSS_DASAR}</style>;
}

// Buang elemen/atribut berbahaya dari HTML bawaan soal upload sebelum
// di-inject (konten admin memang dipercaya, tapi pertahanan lapis kedua
// ini murah: buang script/iframe/handler onclick dll & scheme javascript:).
...
// Pecah teks campur (teks + <table>...</table> / <img ...>) jadi
// segmen-segmen, supaya bagian TEKS tetap bisa dirender LaTeX +
// pre-wrap lewat renderMath(), sedangkan bagian TABEL/GAMBAR dirender
// sebagai HTML sungguhan yang tertata. Tanpa ini, tabel yang menyusup
// di tengah teks soal akan tampil sebagai tag mentah yang jelek.
const REG_TAG_KAYA = /(<table[\s\S]*?<\/table>|<img\b[^>]*\/?>)/gi;
function segmenKaya(teks) {
  if (!teks) return [];
  const sumber = String(teks);
  const out = [];
  const re = new RegExp(REG_TAG_KAYA.source, 'gi');
  let terakhir = 0;
  let m;
  while ((m = re.exec(sumber)) !== null) {
    if (m.index > terakhir) out.push({ jenis: 'teks', isi: sumber.slice(terakhir, m.index) });
    out.push({ jenis: 'html', isi: m[0] });
    terakhir = m.index + m[0].length;
  }
  if (terakhir < sumber.length) out.push({ jenis: 'teks', isi: sumber.slice(terakhir) });
  return out.filter((s) => s.isi && s.isi.trim() !== '');
}

// Render teks yang mungkin mengandung: LaTeX, baris baru, DAN tabel/
// gambar HTML tertanam. Dipakai untuk bacaan, teks soal, pilihan
// jawaban, dan pembahasan -- satu jalur render yang sama supaya
// semuanya konsisten rapi.
function KontenKaya({ teks, style }) {
  if (!teks) return null;
  const seg = segmenKaya(teks);
  if (seg.length === 0) return null;
  return (
    <div style={style}>
      {seg.map((s, i) => (s.jenis === 'teks'
        ? <div key={i} className="gm-teks">{renderMath(s.isi)}</div>
        : (
          <div
            key={i}
            className="gm-html gm-wrap-table"
            dangerouslySetInnerHTML={{ __html: bersihkanHtml(s.isi) }}
          />
        )))}
    </div>
  );
...
// Gambar hasil upload bentuknya macam-macam: string data-URL/URL, atau
// objek {dataUrl|url|uploadedUrl|src}. Samakan jadi array string URL.
function daftarGambar(sumber) {
  if (!sumber) return [];
  const arr = Array.isArray(sumber) ? sumber : [sumber];
  return arr
    .map((g) => (typeof g === 'string' ? g : ((g && (g.uploadedUrl || g.dataUrl || g.url || g.src)) || '')))
    .filter(Boolean);
}

// Deret gambar soal: rata tengah, tinggi dibatasi biar tidak makan
// layar, lazy-load biar halaman tidak berat kalau soalnya banyak gambar.
function GambarSoal({ sumber, maxHeight = 320, prefix = 'Gambar' }) {
  const urls = daftarGambar(sumber);
  if (urls.length === 0) return null;
  return (
    <div style={{ display: 'flex', flexWrap: 'wrap', gap: 10, justifyContent: 'center', alignItems: 'flex-start', margin: '10px 0' }}>
      {urls.map((u, i) => (
        <img key={i} src={u} loading="lazy" alt={`${prefix} ${i + 1}`} className="gm-img" style={{ maxHeight }} />
      ))}
    </div>
  );
}

// Tabel HTML bawaan soal (field tabel_soal dsb.) -- dibungkus wrapper
// scroll horizontal supaya tabel lebar tidak merusak layout mobile.
function TabelSoal({ html }) {
  if (!html) return null;
  return (
    <div className="gm-wrap-table" style={{ margin: '10px 0' }}>
      <div className="gm-html" dangerouslySetInnerHTML={{ __html: bersihkanHtml(html) }} />
    </div>
  );
}

// SATU pintu normalisasi: apa pun bentuk field soal hasil upload,
// keluarannya bentuk kanon yang sama buat dirender & dikoreksi.
function normalisasiSoal(s) {
  if (!s) {
    return { bacaanTeks: '', bacaanGambar: [], teksSoal: '', gambarSoal: [], tabelSoal: '', opsi: [], pembahasan: '', kunci: '', materi: 'Lainnya' };
  }
  const bacaanRaw = (s.bacaan !== undefined && s.bacaan !== null) ? s.bacaan : (s.bacaanTeks || null);
  let bacaanTeks = '';
  let bacaanGambar = [];
  if (typeof bacaanRaw === 'string') {
    bacaanTeks = bacaanRaw;
  } else if (bacaanRaw && typeof bacaanRaw === 'object') {
    bacaanTeks = bacaanRaw.teks || '';
    bacaanGambar = daftarGambar(bacaanRaw.gambar);
  }
  const opsiRaw = s.opsiJawaban || s.opsi || [];
  return {
    bacaanTeks,
    bacaanGambar,
    teksSoal: s.soal || s.teks_soal || s.teksSoal || s.pertanyaan || '',
    gambarSoal: [...daftarGambar(s.gambar), ...daftarGambar(s.gambarUrls), ...daftarGambar(s.images)],
    tabelSoal: s.tabel_soal || s.tabelSoal || s.tabel || '',
    opsi: opsiRaw.map((o) => (typeof o === 'string'
      ? { teks: o, gambar: [] }
      : { teks: (o && (o.teks || o.text)) || '', gambar: daftarGambar(o && (o.gambar || o.gambarUrls)) })),
    pembahasan: s.pembahasan || s.pembahasanTeks || '',
    kunci: s.kunciJawaban || s.kunci_jawaban || s.kunci || '',
    materi: s.materi || s.materiLabel || 'Lainnya',
  };
}
...
export default function LatihanHarianPage() {
  ...
  const [tahap, setTahap] = useState('memuat'); // memuat | pilih-mapel | pilih-mode | mengerjakan | mengirim | mengoreksi | selesai | jatah-habis | khusus-reguler
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        soal = soal.filter((s) => (s.tipe || 'pg_sederhana') === 'pg_sederhana' && (s.opsiJawaban || s.opsi || []).length >= 2);
  ...
  // 🔥🔥 RONDE INI: semua render soal lewat bentuk kanon ini, supaya
  // variasi field hasil upload (teks_soal vs soal, bacaan string vs
  // objek, gambar string vs objek, tabel HTML tertanam, dst.) tidak
  // lagi menghasilkan tampilan berantakan.
  const soalAktifNorm = normalisasiSoal(soalAktif);
  ...
  const jawabDanLanjut = useCallback(() => {
    ...
    // 🔥🔥 RONDE INI: kunci jawaban juga diambil dari bentuk kanon
    // (kunciJawaban / kunci_jawaban / kunci) biar soal upload gaya apa
    // pun tetap terkoreksi benar.
    const indexBenar = hurufKeIndex(soalAktifNorm.kunci);
  ...
  }, [jawabanDipilih, soalAktif, soalAktifNorm.kunci, studentId, progressMap, indexSekarang, soalSesi.length, sedangMenyimpan]);
  ...
  // 🔥🔥 v3 (ronde ini): perapihan render soal upload -- lihat blok
  // "LAPISAN PERAPIHAN SOAL UPLOAD" di atas.
        ...
        <div style={{ padding: 18, marginTop: -6 }}>
          <div style={{ ...st.kartuSoal, borderTop: `4px solid ${ikonMapel.warna}` }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginBottom: 10, flexWrap: 'wrap' }}>
              ...
              <span style={{ fontSize: 11, color: '#94a3b8', fontWeight: 700 }}>{soalAktifNorm.materi}</span>
            </div>
            ...
            {/* 🔥 BARU (BUG SERIUS DITEMUKAN): field `bacaan` sebelumnya
                ...
                SEBELUM pertanyaannya -- persis urutan yang wajar dibaca
                siswa.
                🔥🔥 RONDE INI: bacaan kini dinormalisasi dulu -- bisa
                berupa string polos MAUPUN objek {teks, gambar}, dan
                tabel/gambar HTML di dalamnya ikut tertata rapi. */}
            {(soalAktifNorm.bacaanTeks || soalAktifNorm.bacaanGambar.length > 0) && (
              <div style={st.boxBacaan}>
                <div style={st.labelBacaan}>📖 Bacaan</div>
                <KontenKaya teks={soalAktifNorm.bacaanTeks} style={{ lineHeight: 1.7 }} />
                <GambarSoal sumber={soalAktifNorm.bacaanGambar} maxHeight={260} prefix="Gambar bacaan" />
              </div>
            )}

            {/* 🔥🔥 RONDE INI: teks soal dirender lewat KontenKaya --
                LaTeX ($...$ maupun \(...\)) jalan, baris baru dihormati,
                dan kalau ada <table>/<img> tertanam di teksnya ia jadi
                tabel/gambar beneran, bukan tag mentah. */}
            <div style={{ fontSize: 14.5, color: '#1e293b', marginBottom: 14 }}>
              <KontenKaya teks={soalAktifNorm.teksSoal} />
            </div>
            ...
            {/* 🔥 BARU (celah serius ditemukan): gambar yang nempel
                ...
                🔥🔥 RONDE INI: semua varian field gambar disatukan lewat
                daftarGambar(), ukuran dibatasi & rata tengah biar rapi. */}
            <GambarSoal sumber={soalAktifNorm.gambarSoal} maxHeight={320} />

            {/* 🔥🔥 RONDE INI: tabel data bawaan soal (tabel_soal) --
                dulu tidak dirender sama sekali; sekarang jadi tabel
                bergaris yang bisa di-scroll horizontal di layar sempit. */}
            <TabelSoal html={soalAktifNorm.tabelSoal} />
            ...
            {/* 🔥 ROMBAK: dulu begitu klik "Cek Jawaban", opsi langsung
                ...
                🔥🔥 RONDE INI: opsi yang berisi tabel/gambar HTML (mis.
                opsi pasangan tabel pencernaan) kini dirender rapi lewat
                KontenKaya, bukan string mentah. */}
            {soalAktifNorm.opsi.map((opsi, i) => {
              ...
              return (
                ...
                >
                  <span style={{ width: 26, height: 26, borderRadius: '50%', border: `2px solid ${warna}`, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 11, fontWeight: 800, flexShrink: 0, color: warna, marginTop: 2 }}>
                    {String.fromCharCode(65 + i)}
                  </span>
                  <span style={{ flex: 1, minWidth: 0, display: 'flex', flexDirection: 'column', gap: 6 }}>
                    <KontenKaya teks={opsi.teks} />
                    <GambarSoal sumber={opsi.gambar} maxHeight={140} prefix={`Gambar opsi ${String.fromCharCode(65 + i)}`} />
                  </span>
                </button>
              );
            })}
      ...
      <div style={st.page}>
        ...
        <div style={{ padding: '20px 24px', textAlign: 'center', marginTop: -10 }}>
          ...
          {/* 🔥 BARU: dulu pembahasan & benar/salah tampil LANGSUNG per
              ...
              🔥🔥 RONDE INI: kartu tinjauan juga pakai normalisasi yang
              sama -- bacaan, gambar, tabel, opsi bertabel, dan pembahasan
              LaTeX semuanya tertata rapi seperti waktu mengerjakan. */}
          {hasilPerSoal.length > 0 && (
            <div style={{ textAlign: 'left', marginBottom: 20 }}>
              ...
              {hasilPerSoal.map((item, idx) => {
                const n = normalisasiSoal(item.soal);
                const teksOpsi = (i) => (n.opsi[i] ? n.opsi[i].teks : '');
                return (
                  <div key={idx} style={{ ...st.kartuSoal, marginBottom: 12, textAlign: 'left', borderLeft: `4px solid ${item.benar ? '#22c55e' : '#ef4444'}` }}>
                    ...
                    {/* Bacaan ikut ditampilkan lagi biar tinjauan bisa
                        dibaca berdiri sendiri (soal paham bacaan tidak
                        lagi bikin bingung "teks yang mana?"). */}
                    {(n.bacaanTeks || n.bacaanGambar.length > 0) && (
                      <div style={{ ...st.boxBacaan, marginBottom: 10 }}>
                        <div style={st.labelBacaan}>📖 Bacaan</div>
                        <KontenKaya teks={n.bacaanTeks} style={{ lineHeight: 1.6 }} />
                        <GambarSoal sumber={n.bacaanGambar} maxHeight={200} prefix="Gambar bacaan" />
                      </div>
                    )}

                    <div style={{ fontSize: 13, color: '#1e293b', marginBottom: 10 }}>
                      <KontenKaya teks={n.teksSoal} />
                    </div>
                    <GambarSoal sumber={n.gambarSoal} maxHeight={200} />
                    <TabelSoal html={n.tabelSoal} />

                    <div style={{ fontSize: 12.5, color: '#475569', marginBottom: 4 }}>
                      Jawabanmu: <strong>{String.fromCharCode(65 + item.jawabanIndex)}.</strong> <KontenKaya teks={teksOpsi(item.jawabanIndex)} style={{ display: 'inline' }} />
                    </div>
                    {!item.benar && item.indexBenar >= 0 && (
                      <div style={{ fontSize: 12.5, color: '#16a34a', marginBottom: 4 }}>
                        Kunci: <strong>{String.fromCharCode(65 + item.indexBenar)}.</strong> <KontenKaya teks={teksOpsi(item.indexBenar)} style={{ display: 'inline' }} />
                      </div>
                    )}
                    {n.pembahasan && (
                      <div style={{ ...st.boxPembahasan, marginTop: 10 }}>
                        <b>💡 Pembahasan</b>
                        <div style={{ marginTop: 4 }}><KontenKaya teks={n.pembahasan} /></div>
                      </div>
                    )}
                  </div>
                );
