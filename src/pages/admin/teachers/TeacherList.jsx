...
const TeacherList = () => {
  ...
  // 🔥 TAMBAH GURU (LANGSUNG BUAT AKUN AUTH)
  const handleAddTeacher = async (e) => {
    ...
    try {
      ...
      showAlert(`✅ Guru ${addForm.nama} berhasil ditambahkan! Akun Auth juga dibuat.`);
  ...
  const handleResetPassword = async (email, nama) => {
    ...
    try {
      ...
      if (error.code === 'auth/user-not-found') {
        showAlert("❌ Akun Auth tidak ditemukan.", true);
      } else {
  ...
  const handleDelete = async (id, nama, email) => {
    if (!window.confirm(`Yakin ingin menghapus guru "${nama}"?\n\nCatatan: Akun Auth harus dihapus manual dari Firebase Console.`)) return;
    setDeleting(id);
    try {
      ...
      showAlert(`🗑️ "${nama}" berhasil dihapus dari Firestore. Akun Auth tidak otomatis dihapus.`);
      fetchTeachers();
    } catch (error) { 
  ...
  // 🔥 SIMPAN EDIT (HANYA UPDATE FIRESTORE, TIDAK UPDATE AUTH)
                    ...
                    <tr key={t.id} style={styles.tr}>
                      ...
                      {!isMobile && (
                        <td style={styles.td}>
                          ...
                          {t.authUid ? (
                            <span style={{fontSize: 9, color: '#10b981'}}>✓ Akun Auth aktif</span>
                          ) : (
                            <span style={{fontSize: 9, color: '#f59e0b'}}>⚠️ Perlu reset password</span>
                          )}
                         </td>
                      )}
                      <td style={styles.td}>
            ...
            <div style={styles.modal(isMobile)} onClick={e => e.stopPropagation()}>
              ...
              <form onSubmit={handleAddTeacher} style={styles.modalBody}>
                ...
                <div style={styles.formGroup}>
                  ...
                  <p style={styles.hintText}>Email akan digunakan untuk login otomatis.</p>
                </div>
                <div style={styles.formGroup}>
