...
import {
  ...
  async function transcribeQuestionWithAI(questionImageDataUrl) {
    // 🔥 BARU: endpoint diganti dari /api/smartParseQuiz (Gemini,
    // dipakai bareng fitur SmartImportPanel guru) ke
    // /api/transcribeMathQuestion (Groq, KHUSUS Bank Soal) -- supaya
    // otomatisasi baca-ulang di sini gak numpuk kuota gratis yang
    // sama dengan fitur guru yang jauh lebih sering dipakai.
    const response = await fetch('/api/transcribeMathQuestion', {
      method: 'POST',
  ...
  export default function BankSoalImport({
    ...
    // 🔥 BARU: ref buat autoRetryFlaggedQuestions -- fungsi itu
    // didefinisikan LEBIH BAWAH di file ini (setelah transcribeQuestionWithAI
    // & updateQuestion), tapi perlu dipanggil dari processPages yang
    // posisinya lebih ATAS. Pakai ref (bukan taruh langsung di
    // dependency array useCallback) supaya gak kena error "Cannot
    // access before initialization" -- ref di-.current-kan LEWAT
    // useEffect setelah fungsi aslinya siap, dan processPages cuma
    // baca .current pas benar-benar dipanggil (bukan pas dideklarasikan).
    const autoRetryFlaggedQuestionsRef = useRef(null);
  ...
  const detectQuestionsOnPage = useCallback(async (pageNumber) => {
    ...
    const processPages = useCallback(async () => {
      ...
      for (let pageNumber = from; pageNumber <= to; pageNumber += 1) {
        ...
        try {
          ...
          // 🔥 BARU: otomatiskan baca-ulang AI buat soal yang ditandai
          // possibleMathNotationIssue -- gak nunggu admin klik manual
          // satu-satu. Sengaja TIDAK di-await (biar gak nge-blok proses
          // halaman berikutnya) -- berjalan di latar belakang, hasilnya
          // muncul sebagai panel perbandingan begitu selesai per soal.
          if (autoRetryFlaggedQuestionsRef.current) {
            autoRetryFlaggedQuestionsRef.current(pageNumber, questions);
          }
        } catch (error) {
          setPages((prev) => [
            ...prev,
    ...
    const retryPage = useCallback(
      async (pageNumber) => {
        ...
        try {
          ...
          // 🔥 BARU: sama seperti proses pertama kali -- konsisten kalau
          // admin klik "Ulangi halaman ini" secara manual.
          if (autoRetryFlaggedQuestionsRef.current) {
            autoRetryFlaggedQuestionsRef.current(pageNumber, questions);
          }
        } catch (error) {
    ...
    // 🔥 BARU: OTOMATISASI -- soal yang DITANDAI possibleMathNotationIssue
    // (lihat looksLikeGarbledMathNotation()) langsung dipanggilkan AI
    // baca-ulang OTOMATIS, tanpa admin perlu klik "Coba baca ulang (AI)"
    // satu-satu. Ini permintaan eksplisit: sebelumnya cuma DITANDAI
    // (peringatan visual), sekarang benar-benar DIKERJAKAN otomatis --
    // admin tinggal tinjau perbandingan yang sudah muncul duluan, bukan
    // mulai dari nol per soal.
    //
    // TETAP AMAN: hasilnya tetap masuk sebagai `aiSuggestion` (usulan),
    // BUKAN langsung menimpa -- keputusan akhir (pakai usulan AI atau
    // tetap hasil deterministik) tetap di tangan admin lewat panel
    // perbandingan yang sama seperti retry manual.
    //
    // Dipanggil langsung dari `questions` yang baru selesai diproses
    // (bukan baca ulang dari state `pages`) -- menghindari masalah
    // stale closure sama sekali, karena `updateQuestion` pakai
    // functional setState yang aman dipanggil kapan pun.
    const autoRetryFlaggedQuestions = useCallback(
      async (pageNumber, questions) => {
        const flagged = questions.filter(
          (q) => q.possibleMathNotationIssue && q.rawCropImage,
        );

        for (const q of flagged) {
          if (abortRef.current) break;

          updateQuestion(pageNumber, q.id, {
            aiRetryInProgress: true,
            transcribeError: null,
          });
          ...
          try {
            // eslint-disable-next-line no-await-in-loop
            const transcript = await transcribeQuestionWithAI(q.rawCropImage);
            ...
            updateQuestion(pageNumber, q.id, {
              transcribeError:
                error?.message || 'AI otomatis gagal membaca soal ini.',
              aiRetryInProgress: false,
            });
          }
        }
    ...
    // 🔥 BARU: sinkronkan ref setiap kali fungsi ini berubah -- lihat
    // penjelasan lengkap di deklarasi autoRetryFlaggedQuestionsRef di
    // atas (dekat abortRef/pauseRef).
    useEffect(() => {
      autoRetryFlaggedQuestionsRef.current = autoRetryFlaggedQuestions;
    }, [autoRetryFlaggedQuestions]);
                ...
                <div className="bsi-parsed">
                  ...
                  {selectedPage.questions.map((q, qi) => (
                    ...
                    >
                      ...
                      {/* 🔥 DIPERBARUI: dulu cuma peringatan pasif ("klik
                          Coba baca ulang manual"), SEKARANG AI dipanggil
                          OTOMATIS untuk soal yang ditandai (lihat
                          autoRetryFlaggedQuestions). Pesan ini cuma
                          tampil SELAMA proses otomatis itu berjalan atau
                          kalau ujung-ujungnya tetap gagal -- begitu
                          usulan AI muncul, panel perbandingan di bawah
                          yang jadi fokus, bukan pesan ini lagi. */}
                      {q.possibleMathNotationIssue && !q.aiSuggestion && (
                        <p className="bsi-flag strong">
                          {q.aiRetryInProgress
                            ? '🤖 Kemungkinan notasi matematika (pecahan/matriks) pecah waktu diekstrak -- AI otomatis sedang membaca ulang, tunggu sebentar...'
                            : '⚠️ Kemungkinan notasi matematika (pecahan/matriks/akar bertingkat) pecah waktu diekstrak. AI otomatis sudah mencoba membaca ulang -- kalau tetap gagal (lihat pesan error di bawah), bandingkan dengan crop asli di kiri; kalau memang rusak, lebih aman "Buang" soal ini dan ketik ulang di Word (notasi jadi teks linear, mis. "(x-1+y-1)/(x-1-y-1)").'}
                        </p>
                      )}
