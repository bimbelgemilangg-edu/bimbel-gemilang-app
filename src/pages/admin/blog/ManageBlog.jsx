...
// ─────────────────────────────────────────────────────────────────
// IMAGE COMPRESSOR
// Mengecilkan file size gambar TANPA kehilangan kualitas visual yang
// terlihat — tetap HD di layar, tapi ukuran file jadi jauh lebih kecil
// sehingga upload lebih cepat dan tidak timeout di Firebase.
// ─────────────────────────────────────────────────────────────────
const compressImage = (file, maxWidthPx = 1600, qualityJpeg = 0.88) =>
  new Promise((resolve, reject) => {
    // Kalau bukan gambar, langsung lolos tanpa compress
    ...
    reader.onload = e => {
      const img = new Image();
      img.onerror = () => reject(new Error('Gambar rusak / tidak valid'));
      img.onload = () => {
        // Hitung dimensi baru — jaga aspect ratio, max lebar 1600px
        ...
        // Konversi ke Blob — pakai webp kalau browser support (lebih kecil),
        // fallback ke jpeg
        ...
        canvas.toBlob(
          blob => {
            ...
            // Kalau compressed malah lebih besar (jarang tapi bisa), pakai original
...
// ─────────────────────────────────────────────────────────────────
// UPLOAD ENGINE — dengan compress + retry otomatis (max 3x)
// ─────────────────────────────────────────────────────────────────
  ...
  new Promise(async (resolve, reject) => {
    ...
    for (let attempt = 1; attempt <= 3; attempt++) {
      try {
        ...
        if (attempt < 3) {
          onProgress?.(0); // reset progress bar sebelum retry
          await new Promise(r => setTimeout(r, 1000 * attempt)); // backoff
        }
      }
    }

    reject(lastErr || new Error('Upload gagal setelah 3 percobaan'));
  });
...
const OK = ({ show }) => show
  ? <div style={{ marginTop: 10, padding: '10px 14px', background: 'rgba(37,211,102,0.15)', border: '1px solid rgba(37,211,102,0.3)', borderRadius: 8, color: '#25D366', fontSize: 13 }}>
      ✓ Tersimpan!
    </div>
  : null;
...
const Bar = ({ val, error }) => {
  if (error) return (
    <div style={{ marginTop: 8, padding: '8px 12px', background: 'rgba(231,76,60,0.12)', border: '1px solid rgba(231,76,60,0.3)', borderRadius: 8, color: '#e74c3c', fontSize: 12 }}>
      ⚠️ {error}
    </div>
  );
  ...
  return (
    <div style={{ marginTop: 8 }}>
      <div style={{ height: 6, background: 'rgba(255,255,255,0.08)', borderRadius: 3, overflow: 'hidden' }}>
        <div style={{ height: '100%', width: `${val}%`, background: 'linear-gradient(90deg,#f39c12,#e67e22)', borderRadius: 3, transition: 'width 0.3s' }} />
      </div>
      <div style={{ fontSize: 11, color: C.muted, marginTop: 4 }}>
        Uploading... {val}% {val < 30 ? '(sedang compress gambar)' : val < 90 ? '(mengirim ke Firebase)' : '(hampir selesai)'}
      </div>
    </div>
  );
};
...
const SH = ({ icon, title }) => (
  <div style={{ fontFamily: "'Orbitron',sans-serif", color: C.gold, fontSize: 13, marginBottom: 18, display: 'flex', alignItems: 'center', gap: 8 }}>
    {icon} {title}
  </div>
);
  ...
  return <>
    <div style={cardS}>
      ...
      <Row label="Link TikTok / Instagram">
        <input style={inp} value={url} onChange={e => setUrl(e.target.value)} placeholder="https://www.tiktok.com/@.../video/..." />
      </Row>
      <Row label="Caption">
        <input style={inp} value={caption} onChange={e => setCaption(e.target.value)} placeholder="Tips Belajar Matematika" />
      </Row>
    ...
    <div style={cardS}>
      ...
      {blogs.map(b => (
        <LI key={b.id}>
          <div style={{ width: 36, height: 36, borderRadius: 8, background: b.type === 'tiktok' ? '#111' : 'rgba(131,58,180,0.3)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 16, flexShrink: 0 }}>
            {b.type === 'tiktok' ? '♪' : '◻'}
          </div>
          <div style={{ flex: 1, minWidth: 0 }}>
            <div style={{ fontSize: 13, fontWeight: 600, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{b.caption}</div>
            <div style={{ fontSize: 11, color: C.muted, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{b.url}</div>
          </div>
...
const TabPengajar = ({ teachers }) => {
  ...
  const add = async () => {
    ...
    try {
      ...
      setFile(null); setProg(0); setOk(true);
      setTimeout(() => setOk(false), 2500);
    } catch (e) {
      console.error(e);
      setErr(`Upload gagal: ${e.message}. Coba lagi atau pilih foto yang lebih kecil.`);
      setProg(0);
    }
    setLoading(false);
  };
  ...
  return <>
    <div style={cardS}>
      ...
      <Grid2>
        <Row label="Nama *">
          <input style={inp} value={f.nama} onChange={e => setF({ ...f, nama: e.target.value })} placeholder="Nama lengkap" />
        </Row>
        <Row label="Spesialisasi">
          <input style={inp} value={f.spesialisasi} onChange={e => setF({ ...f, spesialisasi: e.target.value })} placeholder="Guru Matematika" />
        </Row>
      </Grid2>
      <Row label="WhatsApp (628xxx)">
        <input style={inp} value={f.wa} onChange={e => setF({ ...f, wa: e.target.value })} placeholder="6281234567890" />
      </Row>
      <Row label="Bio Singkat">
        <textarea style={{ ...inp, minHeight: 70, resize: 'vertical' }} value={f.bio} onChange={e => setF({ ...f, bio: e.target.value })} placeholder="Pengalaman mengajar..." />
      </Row>
      <Row label="Foto * (otomatis dikompresi, tetap HD)">
        <input type="file" accept="image/*" onChange={e => { setFile(e.target.files[0]); setErr(''); }} style={{ ...inp, padding: '9px 14px' }} />
      </Row>
      <Bar val={prog} error={err} />
      <button style={{ ...btnG, marginTop: 12 }} onClick={add} disabled={loading}>
        {loading ? `Memproses ${prog}%...` : '+ Tambah Pengajar'}
      </button>
      <OK show={ok} />
    </div>
    <div style={cardS}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill,minmax(150px,1fr))', gap: 14 }}>
        {teachers.map(t => (
          <div key={t.id} style={{ textAlign: 'center', background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.08)', borderRadius: 14, padding: 16 }}>
...
const TabBerita = ({ berita }) => {
  ...
  const add = async () => {
    if (!f.judul || !f.isi) return;
    setLoading(true); setErr('');
    let imageUrl = '';
    try {
      if (file) imageUrl = await uploadFile(file, 'berita', setProg);
      ...
      console.error(e);
      setErr(`Upload gambar gagal: ${e.message}`);
      setProg(0);
    }
    setLoading(false);
  };
  ...
  return <>
    <div style={cardS}>
      ...
      <Row label="Judul *">
        <input style={inp} value={f.judul} onChange={e => setF({ ...f, judul: e.target.value })} placeholder="Judul berita" />
      </Row>
      <Row label="Kategori">
        <select style={inp} value={f.kategori} onChange={e => setF({ ...f, kategori: e.target.value })}>
          {['BERITA', 'PENGUMUMAN', 'PRESTASI', 'KEGIATAN', 'PRESS', 'INFO'].map(k => <option key={k}>{k}</option>)}
        </select>
      </Row>
      <Row label="Isi Berita *">
        <textarea style={{ ...inp, minHeight: 130, resize: 'vertical' }} value={f.isi} onChange={e => setF({ ...f, isi: e.target.value })} placeholder="Isi berita lengkap..." />
      </Row>
      <Row label="Foto (opsional — otomatis dikompresi)">
        <input type="file" accept="image/*" onChange={e => { setFile(e.target.files[0]); setErr(''); }} style={{ ...inp, padding: '9px 14px' }} />
      </Row>
      <Bar val={prog} error={err} />
      <button style={{ ...btnG, marginTop: 12 }} onClick={add} disabled={loading}>
        {loading ? `Upload ${prog}%...` : '🚀 Publikasikan'}
      </button>
      <OK show={ok} />
    </div>
    <div style={cardS}>
      ...
      {berita.map(b => (
        <LI key={b.id}>
          ...
          <div style={{ flex: 1, minWidth: 0 }}>
            <div style={{ fontSize: 13, fontWeight: 600, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{b.judul}</div>
            <div style={{ fontSize: 11, color: C.muted }}>{b.kategori} · {b.createdAt?.toDate?.()?.toLocaleDateString('id-ID')}</div>
          </div>
...
const TabTestimoni = ({ testimoni }) => {
  ...
  const add = async () => {
    if (!f.nama || !f.isi) return;
    setLoading(true); setErr('');
    let photoUrl = '';
    try {
      if (file) photoUrl = await uploadFile(file, 'testimoni', setProg);
  ...
  return <>
    <div style={cardS}>
      ...
      <Grid2>
        <Row label="Nama *">
          <input style={inp} value={f.nama} onChange={e => setF({ ...f, nama: e.target.value })} placeholder="Nama siswa / orang tua" />
        </Row>
        <Row label="Kelas / Asal">
          <input style={inp} value={f.kelas} onChange={e => setF({ ...f, kelas: e.target.value })} placeholder="Kelas 9 / Orang Tua" />
        </Row>
      </Grid2>
      <Row label="Isi Testimoni *">
        <textarea style={{ ...inp, minHeight: 90, resize: 'vertical' }} value={f.isi} onChange={e => setF({ ...f, isi: e.target.value })} placeholder="Kesan dan pesan..." />
      </Row>
      <Row label="Rating">
        <div style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
          {[1, 2, 3, 4, 5].map(n => (
            <button key={n} onClick={() => setF({ ...f, bintang: n })} style={{ background: 'none', border: 'none', fontSize: 24, cursor: 'pointer', filter: n <= f.bintang ? 'none' : 'grayscale(1) opacity(0.3)' }}>⭐</button>
          ))}
          <span style={{ color: C.muted, fontSize: 13 }}>{f.bintang}/5</span>
        </div>
      </Row>
      <Row label="Foto (opsional)">
        <input type="file" accept="image/*" onChange={e => { setFile(e.target.files[0]); setErr(''); }} style={{ ...inp, padding: '9px 14px' }} />
      </Row>
      <Bar val={prog} error={err} />
      <button style={{ ...btnG, marginTop: 12 }} onClick={add} disabled={loading}>
        {loading ? `Upload ${prog}%...` : '+ Tambah Testimoni'}
      </button>
      <OK show={ok} />
    </div>
    <div style={cardS}>
      ...
      {testimoni.map(t => (
        <LI key={t.id}>
          {t.photoUrl
            ? <img src={t.photoUrl} alt="" style={{ width: 40, height: 40, borderRadius: '50%', objectFit: 'cover', flexShrink: 0 }} />
            : <div style={{ width: 40, height: 40, borderRadius: '50%', background: 'rgba(243,156,18,0.2)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 18, flexShrink: 0 }}>👤</div>
          }
          <div style={{ flex: 1, minWidth: 0 }}>
            <div style={{ fontSize: 13, fontWeight: 600 }}>{t.nama} <span style={{ color: C.muted, fontWeight: 400 }}>· {t.kelas}</span></div>
...
const TabPenghargaan = ({ penghargaan }) => {
  ...
  const add = async () => {
    ...
    let imageUrl = '';
    try {
      if (file) imageUrl = await uploadFile(file, 'penghargaan', setProg);
      await addDoc(collection(db, 'web_penghargaan'), { ...f, imageUrl, createdAt: serverTimestamp() });
      setF({ judul: '', tahun: '', deskripsi: '' });
      setFile(null); setProg(0); setOk(true);
      setTimeout(() => setOk(false), 2500);
    } catch (e) {
      console.error(e);
  ...
  return <>
    <div style={cardS}>
      ...
      <Grid2>
        <Row label="Nama Penghargaan *">
          <input style={inp} value={f.judul} onChange={e => setF({ ...f, judul: e.target.value })} placeholder="Bimbel Terbaik..." />
        </Row>
        <Row label="Tahun">
          <input style={inp} value={f.tahun} onChange={e => setF({ ...f, tahun: e.target.value })} placeholder="2024" />
        </Row>
      </Grid2>
      <Row label="Deskripsi">
        <input style={inp} value={f.deskripsi} onChange={e => setF({ ...f, deskripsi: e.target.value })} placeholder="Keterangan singkat" />
      </Row>
      <Row label="Foto Piala / Sertifikat">
        <input type="file" accept="image/*" onChange={e => { setFile(e.target.files[0]); setErr(''); }} style={{ ...inp, padding: '9px 14px' }} />
      </Row>
      <Bar val={prog} error={err} />
      <button style={{ ...btnG, marginTop: 12 }} onClick={add} disabled={loading}>
        {loading ? `Upload ${prog}%...` : '+ Tambah Penghargaan'}
      </button>
      <OK show={ok} />
    </div>
    <div style={cardS}>
      ...
      {penghargaan.map(p => (
        <LI key={p.id}>
          {p.imageUrl
            ...
            : <div style={{ width: 44, height: 44, borderRadius: 8, background: 'rgba(243,156,18,0.2)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 22 }}>🏆</div>
          }
          <div style={{ flex: 1 }}>
            <div style={{ fontSize: 13, fontWeight: 600 }}>{p.judul}</div>
            <div style={{ fontSize: 12, color: C.muted }}>{p.tahun} · {p.deskripsi}</div>
          </div>
    ...
    <div style={cardS}>
      ...
      <Row label="Pilih Icon">
        <div style={{ display: 'flex', flexWrap: 'wrap', gap: 8 }}>
          {ICONS_LIST.map(ic => (
            <button key={ic} onClick={() => setF({ ...f, icon: ic })} style={{ width: 40, height: 40, borderRadius: 8, border: f.icon === ic ? '2px solid #f39c12' : '1px solid rgba(255,255,255,0.15)', background: f.icon === ic ? 'rgba(243,156,18,0.15)' : 'transparent', cursor: 'pointer', fontSize: 20 }}>{ic}</button>
          ))}
        </div>
      </Row>
      <Grid2>
        <Row label="Judul *">
          <input style={inp} value={f.judul} onChange={e => setF({ ...f, judul: e.target.value })} placeholder="Materi Lengkap" />
        </Row>
        <Row label="Link (opsional)">
          <input style={inp} value={f.link} onChange={e => setF({ ...f, link: e.target.value })} placeholder="https://... atau kosongkan" />
        </Row>
      </Grid2>
      <Row label="Deskripsi *">
        <textarea style={{ ...inp, minHeight: 80, resize: 'vertical' }} value={f.deskripsi} onChange={e => setF({ ...f, deskripsi: e.target.value })} placeholder="Penjelasan singkat keunggulan" />
      </Row>
      ...
      {keunggulan.map(k => (
        <LI key={k.id}>
          ...
          <div style={{ flex: 1 }}>
            ...
            <div style={{ fontSize: 12, color: C.muted, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{k.deskripsi}</div>
          </div>
  ...
  return <>
    <div style={cardS}>
      ...
      <div style={{ background: 'rgba(37,211,102,0.08)', border: '1px solid rgba(37,211,102,0.2)', borderRadius: 10, padding: '10px 14px', marginBottom: 16, fontSize: 12, color: 'rgba(255,255,255,0.6)' }}>
        💡 Minimal 1 kontak harus ada agar tombol WhatsApp di website aktif.
      </div>
      <Grid2>
        <Row label="Nama *">
          <input style={inp} value={f.nama} onChange={e => setF({ ...f, nama: e.target.value })} placeholder="Nama admin" />
        </Row>
        <Row label="Jabatan">
          <input style={inp} value={f.jabatan} onChange={e => setF({ ...f, jabatan: e.target.value })} placeholder="Admin / Pengajar" />
        </Row>
      </Grid2>
      <Row label="No. WhatsApp * (628xxx)">
        <input style={inp} value={f.wa} onChange={e => setF({ ...f, wa: e.target.value })} placeholder="6281234567890" />
      </Row>
      <Row label="Foto *">
        <input type="file" accept="image/*" onChange={e => { setFile(e.target.files[0]); setErr(''); }} style={{ ...inp, padding: '9px 14px' }} />
      ...
      <Bar val={prog} error={err} />
      <button style={{ ...btnG, marginTop: 12 }} onClick={add} disabled={loading}>
        {loading ? `Upload ${prog}%...` : '+ Tambah Kontak'}
      </button>
      <OK show={ok} />
    </div>
    <div style={cardS}>
      ...
      {contacts.map(c => (
        <LI key={c.id}>
          ...
          <div style={{ flex: 1 }}>
            ...
            <div style={{ fontSize: 12, color: C.muted }}>{c.jabatan} · {c.wa}</div>
          </div>
...
const TabStatistik = ({ settings, setSettings }) => {
  ...
  const fields = [
    ['stat1Num', 'Angka 1', '100+'], ['stat1Label', 'Label 1', 'Siswa Aktif'],
    ['stat2Num', 'Angka 2', '3+'], ['stat2Label', 'Label 2', 'Tahun Berpengalaman'],
    ['stat3Num', 'Angka 3', 'SD–SMA'], ['stat3Label', 'Label 3', 'Jenjang Tersedia'],
    ['stat4Num', 'Angka 4', '95%'], ['stat4Label', 'Label 4', 'Nilai Naik']
  ];
  return (
    <div style={cardS}>
      <SH icon="📊" title="Edit Statistik Halaman Utama" />
      <div style={{ background: 'rgba(243,156,18,0.08)', border: '1px solid rgba(243,156,18,0.2)', borderRadius: 10, padding: '10px 14px', marginBottom: 16, fontSize: 12, color: 'rgba(255,255,255,0.6)' }}>
        Angka-angka ini tampil di bagian bawah hero. Isi dengan data nyata bimbel kamu.
      </div>
      <Grid2>
        {fields.map(([key, label, ph]) => (
          <Row key={key} label={label}>
            <input style={inp} value={settings[key] || ''} onChange={e => setSettings({ ...settings, [key]: e.target.value })} placeholder={ph} />
          </Row>
        ))}
      </Grid2>
...
const TabSettings = ({ settings, setSettings }) => {
  ...
  return <>
    <div style={cardS}>
      ...
      <Grid2>
        <Row label="Nama Bimbel (judul besar)">
          <input style={inp} value={settings.heroTitle || ''} onChange={e => setSettings({ ...settings, heroTitle: e.target.value })} placeholder="GEMILANG" />
        </Row>
        <Row label="Tagline / Sub Judul">
          <input style={inp} value={settings.heroSub || ''} onChange={e => setSettings({ ...settings, heroSub: e.target.value })} placeholder="Eksplorasi Ilmu di Galaksi..." />
        </Row>
      </Grid2>
      <Row label="Visi & Misi">
        <textarea style={{ ...inp, minHeight: 100, resize: 'vertical' }} value={settings.visiMisi || ''} onChange={e => setSettings({ ...settings, visiMisi: e.target.value })} placeholder="Menjadi pusat pembelajaran terbaik..." />
      </Row>
      <Row label="Alamat Lengkap">
        <input style={inp} value={settings.alamat || ''} onChange={e => setSettings({ ...settings, alamat: e.target.value })} placeholder="Jl. Contoh No.1, Desa, Kecamatan" />
      </Row>
    </div>
    <div style={cardS}>
      <SH icon="📱" title="Media Sosial & Maps" />
      <Grid2>
        <Row label="Link TikTok">
          <input style={inp} value={settings.tiktokUrl || ''} onChange={e => setSettings({ ...settings, tiktokUrl: e.target.value })} placeholder="https://tiktok.com/@..." />
        </Row>
        <Row label="Link Instagram">
          <input style={inp} value={settings.igUrl || ''} onChange={e => setSettings({ ...settings, igUrl: e.target.value })} placeholder="https://instagram.com/..." />
        </Row>
      </Grid2>
      <Row label="Link Google Maps">
        <input style={inp} value={settings.mapsUrl || ''} onChange={e => setSettings({ ...settings, mapsUrl: e.target.value })} placeholder="https://maps.google.com/..." />
      </Row>
    </div>
...
export default function ManageBlog() {
  ...
  const [settings, setSettings] = useState({
    ...
    visiMisi: '', mapsUrl: '', tiktokUrl: '', igUrl: '', alamat: '',
    stat1Num: '100+', stat1Label: 'Siswa Aktif',
    stat2Num: '3+', stat2Label: 'Tahun Berpengalaman',
    stat3Num: 'SD–SMA', stat3Label: 'Jenjang Tersedia',
    stat4Num: '95%', stat4Label: 'Nilai Naik'
  });
    ...
    <div style={{ display: 'flex', minHeight: '100vh', background: C.bg, color: 'white', fontFamily: "'Nunito',sans-serif" }}>
      ...
      <div style={{ marginLeft: 240, flex: 1, padding: '24px 28px 60px', overflowY: 'auto' }}>
        <div style={{ marginBottom: 24, paddingBottom: 18, borderBottom: `1px solid ${C.border}`, display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', flexWrap: 'wrap', gap: 14 }}>
          <div>
            <h1 style={{ fontFamily: "'Orbitron',sans-serif", fontSize: 'clamp(15px,2.5vw,20px)', color: '#f39c12', margin: 0 }}>
