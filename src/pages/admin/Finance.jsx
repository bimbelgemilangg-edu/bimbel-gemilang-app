...
import { DollarSign, Plus, Receipt, CheckCircle, X, Trash2, ArrowUpRight, ArrowDownLeft, Wallet, PieChart, FileSpreadsheet, Printer, Edit, Lock, Eye, EyeOff, Calendar, AlertCircle, TrendingUp } from 'lucide-react';
...
export default function AdminFinance({ db }) {
  ...
  return (
    <div className="space-y-10 w-full max-w-[1600px] mx-auto pb-20">
      ...
      {/* 1. PDF PRINT TEMPLATE (TIDAK GEPENG) */}
      <div className="hidden print:block w-full bg-white text-black p-4">
        <div className="text-center border-b-4 border-black pb-4 mb-8">
          <h1 className="text-3xl font-black uppercase">Laporan Keuangan Gemilang</h1>
        ...
        <table className="w-full mb-8 border-collapse">
          <tbody>
            <tr>
              <td className="border-2 border-black p-4 text-center w-1/4"><b>Total Masuk:</b><br/>Rp {formatRupiah(incomeTotal)}</td>
              <td className="border-2 border-black p-4 text-center w-1/4"><b>Total Keluar:</b><br/>Rp {formatRupiah(expenseTotal)}</td>
              <td className="border-2 border-black p-4 text-center w-1/4 bg-gray-100"><b>Saldo Bersih:</b><br/>Rp {formatRupiah(balance.total)}</td>
              <td className="border-2 border-black p-4 text-center w-1/4"><b>Piutang Siswa:</b><br/>Rp {formatRupiah(totalPiutang)}</td>
            </tr>
          </tbody>
        </table>

        <h3 className="font-black mb-2 uppercase text-sm">Rincian Transaksi</h3>
          ...
          <tbody>
            {transactions.map((t, i) => (
              <tr key={i}>
                ...
                <td className="border border-black p-2">{t.date?.toDate ? t.date.toDate().toLocaleDateString() : '-'}</td>
                <td className="border border-black p-2 font-bold">{t.description}</td>
          ...
          <div className="bg-slate-900 p-10 rounded-[3.5rem] text-white shadow-2xl lg:col-span-2 relative overflow-hidden">
            ...
            <div className="grid grid-cols-2 gap-4 pt-6 border-t border-white/10">
               <div className="bg-white/5 p-4 rounded-2xl">
                 <p className="text-[10px] font-black opacity-30 uppercase mb-1">Kas Tunai</p>
                 <p className="text-3xl font-black text-green-400">Rp {formatRupiah(balance.cash)}</p>
               </div>
               <div className="bg-white/5 p-4 rounded-2xl">
                 <p className="text-[10px] font-black opacity-30 uppercase mb-1">Transfer/Bank</p>
                 <p className="text-3xl font-black text-blue-400">Rp {formatRupiah(balance.bank)}</p>
               </div>
            </div>
          </div>
          <div className="bg-white p-10 rounded-[3.5rem] border-4 border-orange-50 shadow-xl flex flex-col justify-center">
            <div className="text-orange-600 mb-2 flex items-center gap-2 font-black uppercase text-xs tracking-widest"><AlertCircle size={24}/> Piutang</div>
            <div className="text-5xl font-black text-orange-700">Rp {formatRupiah(totalPiutang)}</div>
          </div>
          <div className="bg-white p-10 rounded-[3.5rem] border-4 border-green-50 shadow-xl flex flex-col justify-center">
            <div className="text-green-600 mb-2 flex items-center gap-2 font-black uppercase text-xs tracking-widest"><TrendingUp size={24}/> Laba Net</div>
          ...
          {tab === 'summary' && (
            <div className="animate-in fade-in duration-500">
              <div className="p-12 border-b flex justify-between items-center bg-gray-50/50">
                <h3 className="text-4xl font-black text-gray-800 tracking-tighter uppercase">Jurnal Mutasi</h3>
                <button onClick={() => window.print()} className="bg-slate-800 text-white px-12 py-5 rounded-3xl font-black text-xs uppercase tracking-widest shadow-xl flex items-center gap-3 hover:bg-black transition-all">
                  <Printer size={24}/> Cetak PDF
                </button>
              </div>
              <div className="overflow-x-auto">
                <table className="w-full text-left">
                  <thead className="bg-gray-50 text-[10px] font-black uppercase tracking-[0.3em] text-gray-400 border-b">
                    <tr><th className="p-10 text-center">No</th><th className="p-10">Waktu</th><th className="p-10">Keterangan</th><th className="p-10 text-center">Metode</th><th className="p-10 text-right">Nominal</th><th className="p-10 text-center">Owner</th></tr>
                  </thead>
          ...
          {tab === 'invoices' && (
            <div className="p-12 animate-in fade-in duration-500">
              <div className="flex justify-between items-center mb-12">
                <div><h3 className="text-4xl font-black text-gray-800 uppercase tracking-tighter">Hutang Siswa Aktif</h3><p className="text-sm text-gray-400 font-bold uppercase mt-2 italic">Lunas otomatis pindah ke mutasi kas.</p></div>
                <button onClick={()=>setShowManualInv(true)} className="bg-slate-950 text-white px-12 py-6 rounded-[2.5rem] font-black text-sm uppercase tracking-widest shadow-2xl flex gap-4 hover:bg-orange-600 transition-all"><Plus size={24}/> Tambah Tagihan</button>
              </div>
                ...
                {invoices.map(inv => (
                  <div key={inv.id} className="p-12 border-[6px] border-gray-50 rounded-[4rem] bg-white shadow-sm hover:border-orange-200 transition-all">
                    <div className="flex justify-between items-start mb-8">
                      ...
                      <span className="px-6 py-2 rounded-full text-[10px] font-black uppercase bg-red-50 text-red-500 border border-red-100">Tagihan</span>
                    </div>
                    <div className="space-y-4 mb-10">
                      ...
                      <div className="flex items-center gap-2 text-xs font-black text-slate-400 uppercase bg-slate-50 p-4 rounded-2xl border border-slate-100">
                        <Calendar size={18}/> Jatuh Tempo: {inv.dueDate || 'N/A'}
                      </div>
                    </div>
            ...
            <div className="p-12 max-w-5xl mx-auto animate-in slide-in-from-right duration-500">
               <div className="bg-slate-50 p-16 rounded-[4rem] border-[6px] border-white shadow-inner">
                  ...
                  <form onSubmit={handleInputTransaction} className="space-y-12">
                    <div className="space-y-4"><label className="lbl ml-8 tracking-widest">Keterangan</label><input required className="w-full border-[6px] border-white p-8 rounded-[2.5rem] font-black text-3xl outline-none focus:border-blue-600 shadow-xl" value={newTrans.title} onChange={e=>setNewTrans({...newTrans, title:e.target.value})}/></div>
                    <div className="grid grid-cols-2 gap-12">
                      <div className="space-y-4"><label className="lbl ml-8 tracking-widest">Kategori</label><select className="w-full border-[6px] border-white p-8 rounded-[2.5rem] font-black text-2xl outline-none" value={newTrans.category} onChange={e=>setNewTrans({...newTrans, category:e.target.value})}>{inputType==='income'?(<><option>Sponsor</option><option>Hibah</option><option>Lainnya</option></>):(<><option>Gaji Guru</option><option>Operasional</option><option>Listrik/Air</option><option>Lainnya</option></>)}</select></div>
                      <div className="space-y-4"><label className="lbl ml-8 tracking-widest">Akun Kas</label><select className="w-full border-[6px] border-white p-8 rounded-[2.5rem] font-black text-2xl outline-none" value={newTrans.method} onChange={e=>setNewTrans({...newTrans, method:e.target.value})}><option value="Tunai">Tunai</option><option value="Transfer">Bank</option></select></div>
                    </div>
                    <div className="space-y-4"><label className="lbl ml-8 tracking-widest">Nominal</label><input required className="w-full border-[6px] border-white p-10 rounded-[3rem] font-black text-7xl text-blue-600 outline-none shadow-2xl" value={newTrans.amountStr} onChange={e => setNewTrans({...newTrans, amountStr: formatRupiah(e.target.value.replace(/\D/g,""))})}/></div>
      ...
      {showVerifyModal && (
        <div className="fixed inset-0 bg-slate-950/95 z-[500] flex items-center justify-center p-6 backdrop-blur-xl">
          <div className="bg-white w-full max-w-lg rounded-[4rem] p-16 text-center shadow-2xl">
            ...
            <p className="text-xs font-black text-gray-400 uppercase tracking-widest mb-12">Keamanan Duit Terkunci.</p>
            <form onSubmit={verifyOwner} className="space-y-10">
              ...
              <div className="flex gap-4"><button type="button" onClick={()=>setShowVerifyModal(false)} className="flex-1 py-8 rounded-2xl font-black text-gray-400 uppercase bg-gray-100">Batal</button><button type="submit" className="flex-1 bg-red-600 text-white py-8 rounded-2xl font-black uppercase shadow-red-200">Buka</button></div>
            </form>
          </div>
        </div>
      )}

      {/* --- ADD TAGIHAN MODAL (PERBAIKAN ERROR PUTIH) --- */}
      {showManualInv && (
        <div className="fixed inset-0 bg-slate-950/80 z-[500] flex items-center justify-center p-6 backdrop-blur-md">
          <div className="bg-white w-full max-w-2xl rounded-[4rem] p-12 shadow-2xl animate-in zoom-in duration-300">
            ...
            <form onSubmit={handleCreateManualInvoice} className="space-y-8">
              ...
              <input required className="w-full border-4 border-gray-100 p-6 rounded-[2rem] font-black text-2xl shadow-inner outline-none focus:border-blue-600" placeholder="Nominal Rp" value={newInv.totalAmountStr} onChange={e=>setNewInv({...newInv, totalAmountStr: formatRupiah(e.target.value.replace(/\D/g,""))})}/>
              <input className="w-full border-4 border-gray-100 p-6 rounded-[2rem] font-black text-xl outline-none focus:border-blue-600" placeholder="Alasan Tagihan (Misal: Uang Buku)" value={newInv.details} onChange={e=>setNewInv({...newInv, details:e.target.value})}/>
              <div><label className="text-[10px] font-black text-gray-400 uppercase ml-6 mb-2 block">Jatuh Tempo Pembayaran</label><input type="date" required className="w-full border-4 border-gray-100 p-6 rounded-[2rem] font-black text-xl outline-none focus:border-blue-600" value={newInv.dueDate} onChange={e=>setNewInv({...newInv, dueDate:e.target.value})}/></div>
              <button className="w-full bg-slate-950 text-white py-10 rounded-[3rem] font-black text-3xl shadow-2xl">TERBITKAN TAGIHAN</button>
            </form>
          </div>
        </div>
      )}

      {/* --- PAYMENT MODAL (PERBAIKAN ERROR PUTIH) --- */}
      {showPayModal && selectedInvoice && (
        <div className="fixed inset-0 bg-slate-950/80 z-[500] flex items-center justify-center p-6 backdrop-blur-md">
          <div className="bg-white w-full max-w-3xl rounded-[4rem] p-16 shadow-2xl animate-in zoom-in duration-300">
             <div className="flex justify-between items-center mb-12"><h3 className="text-4xl font-black uppercase tracking-tighter">Terima Pembayaran</h3><button onClick={()=>setShowPayModal(false)}><X size={48}/></button></div>
             <form onSubmit={handleProcessPayment} className="space-y-10">
               <div className="bg-blue-50 p-12 rounded-[3.5rem] text-center border-[6px] border-blue-100"><p className="text-xs font-black text-blue-400 uppercase mb-4 tracking-widest">Sisa Hutang</p><p className="text-7xl font-black text-blue-800 tracking-tighter">Rp {formatRupiah(selectedInvoice.remainingAmount)}</p></div>
               <div className="grid grid-cols-2 gap-10">
                 <div className="space-y-2"><label className="text-[10px] font-black text-gray-400 uppercase ml-6">Penerimaan Via</label><select className="w-full border-4 border-gray-100 p-8 rounded-[2.5rem] font-black text-2xl outline-none" value={payMethod} onChange={e=>setPayMethod(e.target.value)}><option value="Tunai">Tunai</option><option value="Transfer">Bank</option></select></div>
                 <div className="space-y-2"><label className="text-[10px] font-black text-gray-400 uppercase ml-6">Nominal Bayar</label><input required className="w-full border-4 border-gray-100 p-8 rounded-[2.5rem] font-black text-3xl shadow-inner outline-none focus:border-blue-600" value={payAmountStr} onChange={e=>setPayAmountStr(formatRupiah(e.target.value.replace(/\D/g,"")))}/></div>
               </div>
               <button className="w-full bg-blue-600 text-white py-10 rounded-[3rem] font-black text-3xl shadow-2xl hover:bg-blue-700 uppercase">Proses</button>
             </form>
          </div>
        </div>
      )}
        ...
        <div className="fixed inset-0 bg-slate-950/80 z-[500] flex items-center justify-center p-6 backdrop-blur-md">
          <div className="bg-white w-full max-w-3xl rounded-[4rem] p-16 shadow-2xl">
            ...
            <form onSubmit={saveEditMutasi} className="space-y-8">
              <div className="space-y-2"><label className="lbl ml-6 tracking-widest">Deskripsi</label><input className="w-full border-4 border-gray-100 p-8 rounded-[2rem] font-black text-2xl outline-none focus:border-blue-600" value={editForm.description} onChange={e=>setEditForm({...editForm, description: e.target.value})}/></div>
              <div className="grid grid-cols-2 gap-8">
                <div className="space-y-2"><label className="lbl ml-6 tracking-widest">Nominal</label><input className="w-full border-4 border-gray-100 p-8 rounded-[2rem] font-black text-2xl outline-none focus:border-blue-600" value={editForm.amountStr} onChange={e=>setEditForm({...editForm, amountStr: formatRupiah(e.target.value.replace(/\D/g,""))})}/></div>
                <div className="space-y-2"><label className="lbl ml-6 tracking-widest">Metode</label><select className="w-full border-4 border-gray-100 p-8 rounded-[2rem] font-black text-2xl outline-none focus:border-blue-600" value={editForm.method} onChange={e=>setEditForm({...editForm, method: e.target.value})}><option>Tunai</option><option>Transfer</option></select></div>
              </div>
              <div className="flex gap-6 pt-10"><button type="button" onClick={()=>setShowEditModal(false)} className="flex-1 py-8 rounded-[2rem] font-black text-gray-400 uppercase bg-gray-50 border-2">Batal</button><button type="submit" className="flex-1 bg-blue-600 text-white py-8 rounded-[2rem] font-black shadow-2xl">Simpan</button></div>
            </form>
          </div>
        </div>
      )}
      ...
      <style>{`
        .lbl { font-size: 10px; font-weight: 900; text-transform: uppercase; color: #94a3b8; }
        @media print {
