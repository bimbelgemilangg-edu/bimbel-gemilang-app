...
import { Trash2, Plus, ArrowLeft, Eye, Upload, Instagram, Box, MoveVertical } from 'lucide-react';
...
const ManagePoster = () => {
  ...
  const [imgPosition, setImgPosition] = useState("center"); // Fitur Baru: Posisi Foto
  ...
  const processSmartUrl = (url) => {
    ...
    try {
      if (url.includes("instagram.com")) {
        ...
        // Menggunakan Proxy Publik + Parameter Media Instagram
  ...
  // --- 2. ENGINE KOMPRESI TINGKAT DEWA (ANTI-LEMOT & ANTI-FULL) ---
  const handleImageChange = (e) => {
    ...
    reader.onload = (event) => {
      ...
      img.onload = () => {
        // Membuat Canvas tak terlihat untuk mengecilkan ukuran tanpa merusak kualitas
        const canvas = document.createElement('canvas');
        const MAX_WIDTH = 1000; // Resolusi maksimal
        ...
        // Kompresi jadi JPEG kualitas 70% (Ukuran file turun drastis hingga 90%)
  ...
  const handleSave = async (e) => {
    ...
    try {
      await addDoc(collection(db, "student_contents"), {
        ...
        content: newContent,
        method: uploadMethod,
        imgPosition: imgPosition, // Simpan posisi grid gambar ke database
        createdAt: serverTimestamp()
      });
      setNewTitle(""); setNewContent(""); setExternalUrl(""); setImagePreview(null); setBase64Image(""); setImgPosition("center");
      fetchPosters();
      alert("✅ Sistem: Data Berhasil Dikompres & Dipublikasikan!");
    } catch (err) {
      console.error(err);
  ...
  // FUNGSI FALLBACK JIKA INSTAGRAM MEMBLOKIR PROXY
  const handleImageError = (e) => {
    if (externalUrl.includes('instagram')) {
      // Jika IG ngeblok, ubah jadi Banner Instagram yang keren
    ...
    <div style={{ padding: '30px', backgroundColor: '#f8fafc', minHeight: '100vh', fontFamily: 'Inter, sans-serif' }}>
      ...
      <div style={{display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '20px'}}>
        <h2 style={{ color: '#1e293b', margin: 0, display: 'flex', alignItems: 'center', gap: '10px' }}>
          <Box size={28} color="#3b82f6" /> Smart Media Engine v2.0
        </h2>
      ...
      <div style={styles.inputCard}>
        ...
        <button onClick={handleSave} disabled={loading} style={styles.btnSave}>
          {loading ? "Menyimpan ke Database..." : "🚀 Luncurkan ke Dashboard Siswa"}
        </button>
      </div>
...
const styles = {
    ...
    textArea: { padding: '14px', borderRadius: '10px', border: '1px solid #cbd5e1', minHeight: '140px', outline: 'none', fontFamily: 'inherit', boxSizing: 'border-box', backgroundColor: '#f8fafc', resize: 'vertical' },
    ...
    posterItem: { background: 'white', borderRadius: '14px', overflow: 'hidden', border: '1px solid #e2e8f0', boxShadow: '0 4px 6px -1px rgba(0,0,0,0.05)', display: 'flex', flexDirection: 'column' },
