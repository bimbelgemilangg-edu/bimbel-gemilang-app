...
const Slider = ({ items, render, height = 400 }) => {
  ...
  if (!items.length) return <div style={{ textAlign: 'center', padding: 40, color: 'rgba(255,255,255,0.3)', fontSize: 13 }}>Belum ada data</div>;
  return <div style={{ position: 'relative', overflow: 'hidden', borderRadius: 16, height, boxShadow: '0 8px 32px rgba(0,0,0,0.2)' }}
    ...
    {items.length > 1 && <>
      <button onClick={() => go((cur - 1 + items.length) % items.length)} style={{ position: 'absolute', left: 10, top: '50%', transform: 'translateY(-50%)', background: 'rgba(0,0,0,0.6)', border: '1px solid rgba(243,156,18,0.3)', color: '#f39c12', width: 36, height: 36, borderRadius: '50%', cursor: 'pointer', fontSize: 18, backdropFilter: 'blur(8px)', zIndex: 2 }}>‹</button>
      <button onClick={() => go((cur + 1) % items.length)} style={{ position: 'absolute', right: 10, top: '50%', transform: 'translateY(-50%)', background: 'rgba(0,0,0,0.6)', border: '1px solid rgba(243,156,18,0.3)', color: '#f39c12', width: 36, height: 36, borderRadius: '50%', cursor: 'pointer', fontSize: 18, backdropFilter: 'blur(8px)', zIndex: 2 }}>›</button>
    </>}
    <div style={{ position: 'absolute', bottom: 12, left: '50%', transform: 'translateX(-50%)', display: 'flex', gap: 6, zIndex: 2 }}>
      {items.map((_, i) => <button key={i} onClick={() => go(i)} style={{ width: i === cur ? 18 : 6, height: 6, borderRadius: 3, background: i === cur ? '#f39c12' : 'rgba(255,255,255,0.3)', border: 'none', cursor: 'pointer', padding: 0, transition: 'all 0.3s' }} />)}
    </div>
  </div>;
};
...
// ════════════ SECTION WRAPPER ════════════
// ============================================================
const Sec = ({ id, children, bg = 'transparent', theme }) => {
  const isDark = theme === 'dark';
  return <section id={id} style={{ 
    position: 'relative', zIndex: 2, padding: '60px 20px', 
    background: bg || (isDark ? 'rgba(5,7,18,0.5)' : 'rgba(255,255,255,0.03)'),
    borderBottom: isDark ? '1px solid rgba(255,255,255,0.05)' : '1px solid rgba(0,0,0,0.05)'
  }}><div style={{ maxWidth: 980, margin: '0 auto' }}>{children}</div></section>;
};

const Title = ({ icon, title, sub, theme }) => {
  const isDark = theme === 'dark';
  return <div style={{ textAlign: 'center', marginBottom: 40 }}>
    ...
    <h2 style={{ 
      ...
      margin: '0 0 8px',
      textShadow: isDark ? '0 0 24px rgba(243,156,18,0.3)' : 'none'
    }}>{title}</h2>
    {sub && <p style={{ color: isDark ? 'rgba(255,255,255,0.4)' : 'rgba(0,0,0,0.5)', fontSize: 13, maxWidth: 480, margin: '0 auto' }}>{sub}</p>}
    <div style={{ width: 48, height: 2, background: `linear-gradient(90deg,transparent,${isDark ? '#f39c12' : '#1a237e'},transparent)`, margin: '12px auto 0' }} />
  </div>;
};
...
// ════════════ DAFTAR MODAL ════════════
// ============================================================
const Modal = ({ contacts, onClose, settings, theme }) => {
  const isDark = theme === 'dark';
  ...
  return <div onClick={onClose} style={{ position: 'fixed', inset: 0, zIndex: 200, background: isDark ? 'rgba(0,0,0,0.92)' : 'rgba(0,0,0,0.7)', backdropFilter: 'blur(8px)', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 16 }}>
    <div onClick={e => e.stopPropagation()} style={{ 
      background: isDark ? 'linear-gradient(135deg,#0d1a35,#0a0f1e)' : 'white',
      border: `1px solid ${isDark ? 'rgba(243,156,18,0.3)' : 'rgba(0,0,0,0.1)'}`,
      borderRadius: 20, padding: '28px 24px', width: '100%', maxWidth: 520, maxHeight: '92vh', overflowY: 'auto',
      boxShadow: isDark ? '0 0 60px rgba(243,156,18,0.08)' : '0 20px 60px rgba(0,0,0,0.15)'
    }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 20 }}>
        <h3 style={{ fontFamily: "'Orbitron',sans-serif", color: isDark ? '#f39c12' : '#1a237e', fontSize: 15, margin: 0 }}>🚀 Formulir Pendaftaran</h3>
...
export default function PublicBlog() {
  ...
  const [settings, setSettings] = useState({
    ...
    founderName: '', founderTitle: '', founderMessage: '', founderPhoto: '',
    ...
    sdJudul: 'SD Kelas 3–6', sdDurasi: '60 menit', sdDeskripsi: 'Fondasi akademik kuat dengan Literasi dan Numerasi.', sdMapel: 'Matematika, IPA, IPS, Bahasa Indonesia, Bahasa Inggris',
    sdFokus1: 'Literasi & Numerasi', sdFokus2: 'Kurikulum Merdeka', sdFokus3: 'Persiapan Ujian',
    smpJudul: 'SMP Kelas 7–9', smpDurasi: '90 menit', smpDeskripsi: 'Penguasaan konsep mendalam dan HOTS.', smpMapel: 'IPA, IPS, Matematika, Bahasa Indonesia, Bahasa Inggris',
    smpFokus1: 'Sinkronisasi Modul', smpFokus2: 'Persiapan UH, PTS, PAT', smpFokus3: 'Persiapan Ujian & Masuk SMA',
    ...
    trialAktif: 'tidak', trialBanner: '', trialDeskripsi: '',
  ...
  return (
    <div style={{ 
      ...
      {/* Latar belakang: Bintang (malam) atau Awan (siang) */}
      {isDark && <Stars />}
      {isDark && <Shooting />}
      ...
      <style>{`
        ...
        *{box-sizing:border-box}
        ...
        .ch:hover{transform:translateY(-4px)!important;box-shadow:0 12px 32px rgba(0,0,0,0.15)!important}
      ...
      <section id="beranda" style={{ 
        ...
        padding: '80px 20px 60px', position: 'relative', zIndex: 2,
        background: isDark ? 'transparent' : 'linear-gradient(135deg,#e8f0fe 0%,#d4e4f7 100%)'
      }}>
        {isDark && (
          <>
            <div style={{ position: 'absolute', right: '5%', top: '15%', opacity: 0.15, pointerEvents: 'none' }} className="nd">
              <AstroFloat size={140} />
            </div>
            <div style={{ position: 'absolute', left: '4%', bottom: '20%', opacity: 0.1, pointerEvents: 'none' }} className="nd">
              <AstroFloat size={90} style={{ animationDelay: '1.5s' }} />
            </div>
          </>
        )}

        <div style={{ animation: 'fadeUp 1s ease both', animationDelay: '0.15s', marginBottom: 20 }}>
          <img 
            src="/logo-gemilang.png.png" 
            alt="Logo Gemilang" 
            style={{ 
              height: isDark ? 110 : 100, 
              filter: isDark ? 'drop-shadow(0 0 28px rgba(243,156,18,0.6))' : 'drop-shadow(0 0 20px rgba(26,35,126,0.2))',
        ...
        <div style={{ animation: 'fadeUp 1s ease both', animationDelay: '0.3s' }}>
          <div style={{ 
            ...
            background: isDark ? 'rgba(243,156,18,0.1)' : 'rgba(26,35,126,0.08)',
            border: `1px solid ${isDark ? 'rgba(243,156,18,0.3)' : 'rgba(26,35,126,0.2)'}`,
            borderRadius: 20, padding: '4px 14px', marginBottom: 16, 
            fontSize: 10, color: accentColor, fontFamily: "'Orbitron',sans-serif", letterSpacing: 0.5 
          }}>✦ BIMBINGAN BELAJAR TERPERCAYA · GLAGAHAGUNG</div>
          <h1 style={{ 
            fontFamily: "'Orbitron',sans-serif", 
            fontSize: 'clamp(26px,6vw,64px)', 
            fontWeight: 900, margin: '0 0 12px', lineHeight: 1.05,
            color: isDark ? 'white' : '#1a1a2e'
          }}>
            SELAMAT DATANG DI<br />
            <span style={{ color: accentColor, textShadow: isDark ? `0 0 32px rgba(243,156,18,0.5)` : `0 0 20px rgba(26,35,126,0.2)` }}>
              {settings.heroTitle}
            </span>
          </h1>
          <p style={{ fontSize: 'clamp(12px,1.8vw,16px)', color: isDark ? 'rgba(255,255,255,0.55)' : 'rgba(0,0,0,0.6)', maxWidth: 520, margin: '0 auto 8px', lineHeight: 1.8 }}>{settings.heroSub}</p>
          {settings.heroDeskripsi && <p style={{ fontSize: 13, color: isDark ? 'rgba(255,255,255,0.35)' : 'rgba(0,0,0,0.45)', maxWidth: 420, margin: '0 auto 28px' }}>{settings.heroDeskripsi}</p>}
        </div>
        ...
        <div style={{ display: 'flex', gap: 10, flexWrap: 'wrap', justifyContent: 'center', animation: 'fadeUp 1s ease both', animationDelay: '0.5s' }}>
          <button onClick={() => setShowDaftar(true)} style={{ 
            ...
            border: 'none', color: '#0a0a1a', padding: '12px 28px', borderRadius: 30, 
            fontWeight: 800, fontSize: 14, cursor: 'pointer', 
            fontFamily: "'Orbitron',sans-serif", 
            boxShadow: '0 0 24px rgba(243,156,18,0.35)',
            animation: 'pulse 2.5s infinite', transition: 'transform 0.2s' 
          }} onMouseEnter={e => e.currentTarget.style.transform = 'scale(1.05)'} onMouseLeave={e => e.currentTarget.style.transform = 'scale(1)'}>
            {settings.heroBtnDaftar}
          </button>
          <button onClick={() => document.getElementById('aktivitas')?.scrollIntoView({ behavior: 'smooth' })} style={{ 
            background: 'transparent', 
            border: `1px solid ${isDark ? 'rgba(243,156,18,0.4)' : 'rgba(26,35,126,0.3)'}`, 
            color: accentColor, padding: '12px 28px', borderRadius: 30, 
            fontSize: 13, cursor: 'pointer', 
            fontFamily: "'Orbitron',sans-serif", transition: 'all 0.2s' 
          }} onMouseEnter={e => e.currentTarget.style.background = isDark ? 'rgba(243,156,18,0.08)' : 'rgba(26,35,126,0.05)'} onMouseLeave={e => e.currentTarget.style.background = 'transparent'}>
            {settings.heroBtnSekunder}
          </button>
        </div>
        ...
        <div style={{ position: 'absolute', bottom: 24, left: '50%', transform: 'translateX(-50%)', display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 4, opacity: 0.4 }}>
          <div style={{ fontSize: 8, fontFamily: "'Orbitron',sans-serif", letterSpacing: 3, color: isDark ? 'rgba(255,255,255,0.5)' : 'rgba(0,0,0,0.4)' }}>SCROLL</div>
          <div style={{ width: 1, height: 30, background: `linear-gradient(180deg,${accentColor},transparent)` }} />
        </div>
      </section>

      {/* ════════════ AKTIVITAS ════════════ */}
      <Sec id="aktivitas" theme={theme}>
        <Title icon="🎬" title={settings.secAktivitasJudul} sub={settings.secAktivitasSub} theme={theme} />
          ...
          return <div style={{ height: '100%', position: 'relative', cursor: 'pointer', borderRadius: 16, overflow: 'hidden', background: isDark ? '#0d1a2e' : '#e8ecf1' }} onClick={() => window.open(item.url, '_blank')}>
            {embed ? <iframe src={embed} style={{ width: '100%', height: '100%', border: 'none', pointerEvents: 'none' }} title="Video" /> :
              <div style={{ height: '100%', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', background: isDark ? 'linear-gradient(135deg,#0d1a35,#1a0a2e)' : 'linear-gradient(135deg,#e8ecf1,#d5dce8)', gap: 12 }}>
                <div style={{ width: 60, height: 60, background: isDark ? 'rgba(243,156,18,0.12)' : 'rgba(26,35,126,0.08)', border: `2px solid ${accentColor}`, borderRadius: '50%', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 24 }}>▶</div>
      ...
      {/* ════════════ PENGAJAR ════════════ */}
      <Sec id="pengajar" theme={theme} bg={isDark ? 'rgba(243,156,18,0.03)' : 'rgba(26,35,126,0.02)'}>
        <Title icon="👨‍🚀" title={settings.secPengajarJudul} sub={settings.secPengajarSub} theme={theme} />
        <Slider height={320} items={teachers} render={t => (
          <div style={{ height: '100%', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
            <div style={{ textAlign: 'center', padding: 24, maxWidth: 340 }}>
              <div style={{ position: 'relative', display: 'inline-block', marginBottom: 14 }}>
                <img src={t.photoUrl} alt={t.nama} style={{ width: 110, height: 110, borderRadius: '50%', objectFit: 'cover', border: `3px solid ${accentColor}`, boxShadow: `0 0 28px ${isDark ? 'rgba(243,156,18,0.25)' : 'rgba(26,35,126,0.15)'}` }} />
              ...
              <h3 style={{ fontFamily: "'Orbitron',sans-serif", fontSize: 16, color: 'white', margin: '0 0 4px' }}>{t.nama}</h3>
      ...
      {/* ════════════ KEUNGGULAN ════════════ */}
      {keunggulan.length > 0 && (
        <Sec id="keunggulan" theme={theme}>
          <Title icon="✨" title="KEUNGGULAN KAMI" sub="Mengapa keluarga memilih Bimbel Gemilang" theme={theme} />
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill,minmax(210px,1fr))', gap: 16 }}>
            {keunggulan.map(k => (
              <div key={k.id} className="ch" onClick={() => k.link && window.open(k.link, '_blank')} style={{ 
                background: cardBg, border: `1px solid ${borderColor}`, borderRadius: 14, padding: 22, 
                cursor: k.link ? 'pointer' : 'default', boxShadow: isDark ? '0 4px 20px rgba(0,0,0,0.2)' : '0 4px 16px rgba(0,0,0,0.04)'
              }}>
                <div style={{ fontSize: 30, marginBottom: 10 }}>{k.icon}</div>
                <h3 style={{ fontFamily: "'Orbitron',sans-serif", fontSize: 12, color: textColor, margin: '0 0 8px' }}>{k.judul}</h3>
                <p style={{ fontSize: 12, color: isDark ? 'rgba(255,255,255,0.5)' : 'rgba(0,0,0,0.55)', lineHeight: 1.7, margin: 0 }}>{k.deskripsi}</p>
              </div>
            ))}
          </div>
        </Sec>
      ...
      {/* ════════════ PROGRAM ════════════ */}
      <Sec id="program" theme={theme} bg={isDark ? 'rgba(124,58,237,0.04)' : 'rgba(26,35,126,0.02)'}>
        <Title icon="📖" title="PROGRAM KAMI" sub="Dirancang sesuai kebutuhan akademis SD dan SMP" theme={theme} />
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(280px,1fr))', gap: 20 }}>
          {/* SD */}
          <div className="ch" style={{ 
            background: isDark ? 'linear-gradient(135deg,rgba(243,156,18,0.08),rgba(124,58,237,0.05))' : 'white',
            border: `1px solid ${isDark ? 'rgba(243,156,18,0.2)' : 'rgba(26,35,126,0.1)'}`,
            borderRadius: 16, padding: 24, boxShadow: isDark ? '0 8px 32px rgba(0,0,0,0.2)' : '0 4px 20px rgba(0,0,0,0.04)'
          }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 14 }}>
              <div style={{ width: 46, height: 46, borderRadius: 12, background: isDark ? 'rgba(243,156,18,0.15)' : 'rgba(26,35,126,0.08)', border: `1px solid ${isDark ? 'rgba(243,156,18,0.3)' : 'rgba(26,35,126,0.15)'}`, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 20 }}>🏫</div>
            ...
            <div style={{ marginBottom: 14 }}>
              ...
              <div style={{ display: 'flex', flexWrap: 'wrap', gap: 5 }}>
                {settings.sdMapel.split(',').map(m => <span key={m} style={{ fontSize: 10, padding: '2px 8px', borderRadius: 10, background: isDark ? 'rgba(243,156,18,0.1)' : 'rgba(26,35,126,0.06)', border: `1px solid ${isDark ? 'rgba(243,156,18,0.2)' : 'rgba(26,35,126,0.1)'}`, color: accentColor }}>{m.trim()}</span>)}
              </div>
            </div>
            <div>
          ...
          {/* SMP */}
          <div className="ch" style={{ 
            background: isDark ? 'linear-gradient(135deg,rgba(124,58,237,0.08),rgba(243,156,18,0.05))' : 'white',
            border: `1px solid ${isDark ? 'rgba(124,58,237,0.25)' : 'rgba(26,35,126,0.1)'}`,
            borderRadius: 16, padding: 24, boxShadow: isDark ? '0 8px 32px rgba(0,0,0,0.2)' : '0 4px 20px rgba(0,0,0,0.04)'
          }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 14 }}>
              <div style={{ width: 46, height: 46, borderRadius: 12, background: isDark ? 'rgba(124,58,237,0.15)' : 'rgba(26,35,126,0.08)', border: `1px solid ${isDark ? 'rgba(124,58,237,0.3)' : 'rgba(26,35,126,0.15)'}`, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 20 }}>🎓</div>
            ...
            <div style={{ marginBottom: 14 }}>
              <div style={{ fontSize: 9, color: isDark ? 'rgba(255,255,255,0.3)' : 'rgba(0,0,0,0.3)', fontFamily: "'Orbitron',sans-serif", letterSpacing: 0.5, marginBottom: 6 }}>MATA PELAJARAN</div>
              <div style={{ display: 'flex', flexWrap: 'wrap', gap: 5 }}>
                {settings.smpMapel.split(',').map(m => <span key={m} style={{ fontSize: 10, padding: '2px 8px', borderRadius: 10, background: isDark ? 'rgba(124,58,237,0.12)' : 'rgba(26,35,126,0.06)', border: `1px solid ${isDark ? 'rgba(124,58,237,0.25)' : 'rgba(26,35,126,0.1)'}`, color: isDark ? '#a78bfa' : '#1a237e' }}>{m.trim()}</span>)}
              </div>
            </div>
            <div>
              <div style={{ fontSize: 9, color: isDark ? 'rgba(255,255,255,0.3)' : 'rgba(0,0,0,0.3)', fontFamily: "'Orbitron',sans-serif", letterSpacing: 0.5, marginBottom: 6 }}>FOKUS</div>
        ...
        <div style={{ marginTop: 32, background: isDark ? 'rgba(255,255,255,0.03)' : 'rgba(0,0,0,0.02)', border: `1px solid ${borderColor}`, borderRadius: 14, padding: '20px 24px' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill,minmax(180px,1fr))', gap: 10 }}>
            {fasilitas.map((f, i) => <div key={i} style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '8px 12px', background: isDark ? 'rgba(255,255,255,0.04)' : 'rgba(0,0,0,0.02)', borderRadius: 8, border: `1px solid ${borderColor}` }}>
      ...
      {/* ════════════ BERITA ════════════ */}
      <Sec id="berita" theme={theme} bg={isDark ? 'rgba(139,92,246,0.03)' : 'rgba(26,35,126,0.01)'}>
        <Title icon="📡" title={settings.secBeritaJudul} sub={settings.secBeritaSub} theme={theme} />
        {berita.length > 0 ? <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill,minmax(260px,1fr))', gap: 16 }}>
          {berita.map(b => <div key={b.id} className="ch" onClick={() => setSelBerita(b)} style={{ background: cardBg, border: `1px solid ${borderColor}`, borderRadius: 14, overflow: 'hidden', cursor: 'pointer', boxShadow: isDark ? '0 4px 20px rgba(0,0,0,0.2)' : '0 4px 16px rgba(0,0,0,0.04)' }}>
            ...
            <div style={{ padding: 16 }}>
              ...
              <h3 style={{ fontSize: 13, fontWeight: 700, color: textColor, margin: '5px 0', lineHeight: 1.4 }}>{b.judul}</h3>
      ...
      {/* ════════════ TESTIMONI ════════════ */}
      {testimoni.length > 0 && (
        <Sec id="testimoni" theme={theme} bg={isDark ? 'rgba(37,211,102,0.02)' : 'rgba(26,35,126,0.01)'}>
          <Title icon="⭐" title="TESTIMONI" sub="Apa kata mereka tentang Bimbel Gemilang" theme={theme} />
            ...
            <div style={{ height: '100%', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '0 16px' }}>
              <div style={{ background: isDark ? 'rgba(255,255,255,0.04)' : 'white', border: `1px solid ${borderColor}`, borderRadius: 16, padding: '22px 28px', maxWidth: 520, width: '100%', textAlign: 'center' }}>
                ...
                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 10 }}>
                  {t.photoUrl ? <img src={t.photoUrl} alt={t.nama} style={{ width: 36, height: 36, borderRadius: '50%', objectFit: 'cover', border: `2px solid ${accentColor}` }} /> : <div style={{ width: 36, height: 36, borderRadius: '50%', background: isDark ? 'rgba(243,156,18,0.15)' : 'rgba(26,35,126,0.08)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 16 }}>👤</div>}
                  <div style={{ textAlign: 'left' }}><div style={{ fontSize: 13, fontWeight: 700, color: textColor }}>{t.nama}</div><div style={{ fontSize: 10, color: isDark ? 'rgba(255,255,255,0.35)' : 'rgba(0,0,0,0.4)' }}>{t.kelas}</div></div>
                </div>
              </div>
            </div>
          )} />
      ...
      {/* ════════════ KONTAK ════════════ */}
      <Sec id="kontak" theme={theme}>
        <Title icon="📡" title={settings.secKontakJudul} sub={settings.secKontakSub} theme={theme} />
        {contacts.length > 0 ? <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill,minmax(240px,1fr))', gap: 14, marginBottom: 28 }}>
          {contacts.map(c => <div key={c.id} className="ch" onClick={() => window.open(`https://wa.me/${c.wa}`, '_blank')} style={{ background: isDark ? 'rgba(37,211,102,0.07)' : 'white', border: `1px solid ${isDark ? 'rgba(37,211,102,0.2)' : 'rgba(0,0,0,0.06)'}`, borderRadius: 14, padding: 14, cursor: 'pointer', display: 'flex', alignItems: 'center', gap: 12 }}>
            ...
            <div><div style={{ fontWeight: 700, fontSize: 13 }}>{c.nama}</div><div style={{ fontSize: 11, color: '#25D366' }}>● Online · Chat WA</div></div>
          </div>)}
      ...
      {/* MODALS */}
      {showDaftar && <Modal contacts={contacts} onClose={() => setShowDaftar(false)} settings={settings} theme={theme} />}
      {selBerita && <div onClick={() => setSelBerita(null)} style={{ position: 'fixed', inset: 0, zIndex: 200, background: isDark ? 'rgba(0,0,0,0.92)' : 'rgba(0,0,0,0.7)', backdropFilter: 'blur(8px)', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 16 }}>
        <div onClick={e => e.stopPropagation()} style={{ 
          background: isDark ? 'linear-gradient(135deg,#0d1a35,#0a0f1e)' : 'white',
          border: `1px solid ${isDark ? 'rgba(243,156,18,0.25)' : 'rgba(0,0,0,0.08)'}`,
          borderRadius: 16, padding: 24, maxWidth: 540, width: '100%', maxHeight: '88vh', overflowY: 'auto'
        }}>
...
const Nav = ({ onDaftar, settings, theme, toggleTheme }) => {
  ...
  return <nav style={{ 
    ...
    <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
      <img src="/logo-gemilang.png.png" alt="Logo" style={{ height: 34, filter: isDark ? 'drop-shadow(0 0 10px rgba(243,156,18,0.4))' : 'none', mixBlendMode: isDark ? 'screen' : 'normal' }} onError={e => { e.target.style.display = 'none'; }} />
    </div>
    ...
    <div style={{ display: 'flex', gap: 2, alignItems: 'center' }} className="nd">
      {links.map(([id, label]) => <button key={id} onClick={() => go(id)} style={{ 
        ...
        fontWeight: sc ? 500 : 400
      }} onMouseEnter={e => { e.target.style.color = isDark ? '#f39c12' : '#1a237e'; e.target.style.background = isDark ? 'rgba(243,156,18,0.08)' : 'rgba(26,35,126,0.06)'; }} onMouseLeave={e => { e.target.style.color = isDark ? 'rgba(255,255,255,0.7)' : 'rgba(0,0,0,0.6)'; e.target.style.background = 'none'; }}>{label}</button>)}
