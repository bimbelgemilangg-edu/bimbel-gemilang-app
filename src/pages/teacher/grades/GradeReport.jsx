...
const GradeReport = () => {
  ...
  // DATA HASIL OLAHAN
  const [reportData, setReportData] = useState([]); // Detail Nilai
  const [subjectAverages, setSubjectAverages] = useState([]); // Grafik Nilai
  const [attitudeData, setAttitudeData] = useState([]); // Grafik Radar
  ...
  // 2. ENGINE PENGOLAH NILAI (SMART LOGIC)
  useEffect(() => {
    ...
    const processGrades = async () => {
        ...
        // A. HITUNG RATA-RATA PER MAPEL (ANTI-TEKOR LOGIC)
        const mapelGroups = {};
        ...
        setSubjectAverages(averages); // Data untuk Grafik Bar

        // B. HITUNG GPA (Hanya dari mapel yang ada nilainya)
        const finalGpa = subjectsCount > 0 ? Math.round(totalSum / subjectsCount) : 0;
        setGpa(finalGpa);

        // C. HITUNG RATA-RATA SIKAP (RADAR CHART)
        ...
        // D. GENERATE SMART ANALYSIS TEXT
