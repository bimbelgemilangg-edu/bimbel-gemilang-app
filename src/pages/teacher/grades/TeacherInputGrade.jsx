...
const TeacherInputGrade = () => {
  ...
  const [aspects, setAspects] = useState({
    pemahaman: 3, // Cognitive
    aplikasi: 3,  // Problem Solving
    literasi: 3,  // Critical Thinking
    inisiatif: 3, // Active Learning
    mandiri: 3    // Self-Regulated
  });

  // Tentukan Mapel Guru (Otomatis)
  const defaultMapel = guru?.program === 'English' ? 'Bahasa Inggris' : 
                       guru?.program === 'Matematika' ? 'Matematika' : 
                       'Tematik SD'; // Default fallback
  const [selectedMapel, setSelectedMapel] = useState(defaultMapel);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
        await addDoc(collection(db, "grades"), {
            ...
            mapel: selectedMapel,
            topik: topic,
            nilai: parseInt(score), // Akademik
            qualitative: aspects,   // 5 Aspek Karakter
  ...
  // KOMPONEN RATING BINTANG/SLIDER
  ...
  return (
    <div style={{padding:20, maxWidth:800, margin:'0 auto', fontFamily:'sans-serif'}}>
      ...
      <div style={{background:'white', padding:20, borderRadius:10, boxShadow:'0 2px 5px rgba(0,0,0,0.1)', marginBottom:20}}>
        ...
        <p style={{margin:0, fontSize:13, color:'#7f8c8d'}}>Guru: {guru?.nama} | Mapel Fokus: <b>{selectedMapel}</b></p>
      ...
      {selectedStudent ? (
        ...
        <div style={{background:'white', padding:25, borderRadius:10, boxShadow:'0 5px 15px rgba(0,0,0,0.1)'}}>
            ...
            <form onSubmit={handleSubmit}>
                {/* 1. AKADEMIK */}
                <h4 style={{background:'#eef2f3', padding:8, borderRadius:5}}>A. Capaian Akademik</h4>
                ...
                <RenderRating 
                    label="1. Pemahaman Konsep (Cognitive)" 
                    desc="Sejauh mana siswa memahami dasar teori pelajaran." 
                    value={aspects.pemahaman} field="pemahaman" 
                />
                <RenderRating 
                    label="2. Logika & Aplikasi (Problem Solving)" 
                    desc="Kemampuan menerapkan konsep pada soal cerita/kasus." 
                    value={aspects.aplikasi} field="aplikasi" 
                />
                <RenderRating 
                    label="3. Literasi & Analisis (Critical Thinking)" 
                    desc="Kemampuan membaca dan menganalisis informasi soal." 
                    value={aspects.literasi} field="literasi" 
                />
                <RenderRating 
                    label="4. Inisiatif Belajar (Active Learning)" 
                    desc="Keaktifan bertanya dan antusiasme di kelas." 
                    value={aspects.inisiatif} field="inisiatif" 
                />
                <RenderRating 
                    label="5. Kemandirian (Self-Regulated)" 
                    desc="Mampu mengerjakan tugas tanpa perlu diarahkan terus." 
                    value={aspects.mandiri} field="mandiri" 
                />
