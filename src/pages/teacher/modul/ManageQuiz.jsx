...
// Import KaTeX untuk render matematika
...
const ManageQuiz = () => {
  ...
  // Helper untuk merender teks yang mengandung LaTeX
  ...
  const handleSmartGenerate = () => {
    ...
    setQuestions(questions[0]?.q === "" ? parsedQuestions : [...questions, ...parsedQuestions]);
    setBulkText("");
    setShowImport(false);
  };
  ...
  const handleSaveQuiz = async () => {
    ...
    try {
      ...
      alert("✅ KUIS MATEMATIKA BERHASIL DISIMPAN!");
      navigate(-1);
    ...
    <div style={st.container(isMobile)}>
        ...
        <div style={st.modalOverlay}>
          <div style={st.modalContent(isMobile)}>
            <div style={st.modalHeader}>
              <h3 style={{margin:0, display:'flex', alignItems:'center', gap:10}}><Wand2 size={20} color="#673ab7"/> Smart Math Importer</h3>
              <button onClick={() => setShowImport(false)} style={st.btnClose}><X/></button>
            </div>
            <div style={st.mathHint}>
              Gunakan tanda <b>$</b> untuk rumus. Contoh: <b>$\frac{1}{2}$</b> atau <b>$x^2$</b>
            </div>
            <textarea 
              style={st.bulkArea} 
              placeholder="1. Hitunglah $\sqrt{144}$&#10;A. 10&#10;B. 12&#10;C. 14"
              value={bulkText}
              onChange={(e) => setBulkText(e.target.value)}
            />
            <button onClick={handleSmartGenerate} style={st.btnGenerate}>GENERATE SOAL</button>
          </div>
        </div>
      )}
      ...
      <div style={st.headerNav}>
        ...
        <div style={{display:'flex', gap:10}}>
           <button onClick={() => setShowImport(true)} style={st.btnImport}><Calculator size={18}/> Bulk Math</button>
      ...
      {questions.map((item, idx) => (
        <div key={item.id} style={st.qCard}>
          ...
          />
          {/* PRIVEW MATEMATIKA OTOMATIS */}
          <div style={st.mathPreview}>
             <span style={{fontSize:10, color:'#94a3b8', display:'block'}}>Preview Soal:</span>
             {renderMath(item.q)}
          </div>
            ...
            {item.options.map((opt, optIdx) => (
              ...
              >
                ...
                <div style={{flex:1}}>
                    ...
                    />
                    <div style={{fontSize:12, marginTop:4}}>{renderMath(opt)}</div>
                </div>
...
const st = {
  ...
  dateInput: { width: '100%', padding: '12px', borderRadius: '10px', border: '1px solid #e2e8f0' },
  qCard: { background: 'white', width: '100%', maxWidth: '800px', padding: '25px', borderRadius: '24px', marginBottom: '20px', border: '1px solid #e2e8f0' },
  ...
  qInput: { width: '100%', minHeight: '60px', padding: '15px', borderRadius: '15px', border: '1px solid #f1f5f9', background: '#f8fafc', fontSize: '14px', fontWeight: '600', outline: 'none' },
  mathPreview: { padding: '10px 15px', background: '#f1f5f9', borderRadius: '12px', marginBottom: 15, fontSize: '15px', minHeight: '40px' },
  ...
  optItem: (act) => ({ 
    ...
    borderColor: act ? '#10b981' : '#f1f5f9', background: act ? '#f0fdf4' : '#fff', cursor: 'pointer'
  }),
  ...
  modalOverlay: { position: 'fixed', top: 0, left: 0, width: '100%', height: '100%', background: 'rgba(0,0,0,0.6)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 9999, padding: '20px' },
  modalContent: (m) => ({ background: 'white', width: '100%', maxWidth: '700px', padding: '30px', borderRadius: '28px' }),
  modalHeader: { display: 'flex', justifyContent: 'space-between', marginBottom: 10 },
  mathHint: { background: '#eef2ff', padding: '10px', borderRadius: '10px', fontSize: '11px', color: '#4338ca', marginBottom: 10 },
