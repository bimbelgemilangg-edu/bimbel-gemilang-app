...
const ManageQuiz = () => {
  ...
  // Ambil ID Modul Materi dari URL (?modulId=xxx)
  ...
  // --- 1. LOAD DATA KUIS DARI MATERI TERKAIT ---
  useEffect(() => {
    if (modulId) {
      const fetchCurrentData = async () => {
        try {
          ...
          if (snap.exists()) {
            ...
            // Jika sudah ada soal sebelumnya, masukkan ke state
  ...
  // --- 2. LOGIKA INPUT (SOLUSI ANTI-MACET) ---
  const addQuestion = () => {
  ...
  // --- 3. SIMPAN KE FIRESTORE (SINKRON KE MATERI) ---
  const handleSaveQuiz = async () => {
    ...
    try {
      ...
      await updateDoc(docRef, {
        // Simpan sebagai array quizData di dalam dokumen materi
      ...
      alert("🚀 Kuis Berhasil Disinkronkan ke Materi!");
      navigate(-1); // Kembali ke halaman ManageMateri
    } catch (err) {
  ...
  return (
    <div style={styles.container}>
      ...
      <div style={styles.headerNav}>
        <button onClick={() => navigate(-1)} style={styles.btnBack}><ArrowLeft size={18}/> Batal & Kembali</button>
        <button onClick={handleSaveQuiz} disabled={loading} style={styles.btnSave}>
          <Save size={18}/> {loading ? "Menyimpan..." : "Update Kuis"}
        </button>
      </div>
      ...
      <div style={styles.infoCard}>
        ...
        <div style={styles.deadlineBox}>
           <label style={styles.label}>Batas Waktu Pengerjaan (Opsional):</label>
           <input 
             type="datetime-local" 
      ...
      {!modulId && (
        <div style={styles.errorBanner}>
          <AlertCircle size={20}/> 
          Peringatan: Editor ini harus dibuka melalui halaman Materi agar tersimpan dengan benar.
        </div>
      )}
    </div>
  );
...
const styles = {
  ...
  headerNav: { width: '100%', maxWidth: '750px', display: 'flex', justifyContent: 'space-between', marginBottom: '20px' },
  ...
  infoCard: { background: 'white', width: '100%', maxWidth: '750px', padding: '25px', borderRadius: '24px', marginBottom: '25px', border: '1px solid #e2e8f0' },
  ...
  btnAdd: { width: '100%', maxWidth: '750px', padding: '18px', border: '2px dashed #cbd5e1', color: '#64748b', background: 'white', borderRadius: '20px', fontWeight: '800', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 10 },
